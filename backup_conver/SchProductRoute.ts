...
class SchProductRoute {
    ...
    if (this.cVersionNo.toLowerCase() == 'sureversion') {
      ...
      if (
        ...
        for (let i = 0; i < ListRouteRes2.length; i++) {
          ...
          ListRouteRes2[i].dResEndDate = new Date(dCanBegDate.getTime() + 60000)
      ...
      for (let i = 0; i < ListRouteRes.length; i++) {
        if (ListRouteRes[i].iResReqQty > 0)
          ...
          ListRouteRes[i].dResEndDate = new Date(dCanBegDate.getTime() + 60000)
      ...
      try {
        if (this.iReqQty > 0) {
          ...
          if (
            ...
            for (let i = 0; i < ListRouteRes2.length; i++) {
              ListRouteRes2[i].dResBegDate = dCanBegDate
              ListRouteRes2[i].dResEndDate = new Date(
      ...
      for (let i = 0; i < ListRouteRes.length; i++) {
        ...
        try {
          ...
          const ldtEndedDate = new Date()
          const iWaitTime2 = new Date().getTime() - ldtBeginDate.getTime()
  ...
  ResourceSelect(dCanBegDate: Date, bFreeze: boolean = false): number {
    ...
    if (iSelectReturn <= 0) {
      if (SchParam.cMutResourceType == '4') {
        ...
        if (ListRouteRes.length > 1) {
          for (let i = 0; i < ListRouteRes.length; i++) {
            if (ListRouteRes[i].cCanScheduled != '1') continue
            ListRouteRes[i].cDefine38 = new Date(
              ListRouteRes[i].dResEndDate.getTime() +
  ...
  ResourceSelectRev(dCanBegDate: Date, bFreeze: boolean = false): number {
    let ListRouteRes = this.SchProductRouteResList.filter(
      (p) => p.cSelected == '1',
    )
    if (bFreeze || this.iActQty > 0) {
  ...
  ProcessSchTaskRev(bSet: string = '0'): number {
    ...
    try {
      if (this.SchProductRouteNextList.length > 0) {
        ...
        dCanEndDate = this.schProduct.dDeliveryDate.toJSDate()
        if (dCanEndDate <= this.schData.dtStart) {
