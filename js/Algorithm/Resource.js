...
class Resource extends Base_1.Base {
    constructor(cResourceNo) {
        super();
        this.bScheduled = 0;
        ...
        this.ResTimeRangeListBak = new Array(10);
        this.ResSpecTimeRangeList = new Array(10);
        this.schProductRouteResList = new Array(10);
        if (cResourceNo) {
            const schData = this.schData;
            const dr = schData.dtResource.filter((row) => row.cResourceNo === cResourceNo);
            if (dr.length < 1)
                return;
    ...
    get ResWorkCenter() {
        if (this.cWcNo === '')
            this.resWorkCenter = null;
        const listWorkCenter = this.schData.WorkCenterList.filter((p1) => p1.cWcNo === this.cWcNo);
    ...
    getNotSchTask() {
        this.schProductRouteResList = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
            p1.BScheduled === 0 &&
    ...
    mergeTimeRange() {
        ...
        let resSpecTimeRangeList1 = this.ResSpecTimeRangeList.filter((p1) => p1.Attribute === TimeRangeAttribute.Work ||
            p1.Attribute === TimeRangeAttribute.Overtime ||
            p1.Attribute === TimeRangeAttribute.MayOvertime);
        resSpecTimeRangeList1.sort((p1, p2) => p1.DBegTime.getTime() - p2.DBegTime.getTime());
        ...
        for (const resTimeRange of resSpecTimeRangeList1) {
            ...
            let lResTimeRangeList1 = this.ResTimeRangeList.filter((p2) => p2.DBegTime <= resTimeRange.DBegTime);
            lResTimeRangeList1.sort((p1, p2) => p2.DBegTime.getTime() - p1.DBegTime.getTime());
            if (lResTimeRangeList1.length > 0) {
                dCanBegDate = lResTimeRangeList1[0].DBegTime;
            }
            let lResTimeRangeList = this.ResTimeRangeList.filter((p2) => p2.DBegTime >= dCanBegDate);
            lResTimeRangeList.sort((p1, p2) => p1.DBegTime.getTime() - p2.DBegTime.getTime());
            let resLastTimeRange = null;
            const iCount = lResTimeRangeList.length;
            for (let i = 0; i < iCount; i++) {
                const resWorkTimeRange = lResTimeRangeList[i];
                if (i > 0) {
                    const resAddTimeRange = new type_1.ResTimeRange();
        ...
        // let TimeRangeAttribute=this.TimeRangeAttribute
        let resSpecTimeRangeList2 = this.ResSpecTimeRangeList.filter((p1) => p1.Attribute === TimeRangeAttribute.Maintain ||
            p1.Attribute === TimeRangeAttribute.Snag);
        resSpecTimeRangeList2.sort((p1, p2) => p1.DBegTime.getTime() - p2.DBegTime.getTime());
    ...
    mergeTimeRangeSub(resWorkTimeRange, resSpecTimeRange) {
        const resAddTimeRange = new type_1.ResTimeRange();
        resAddTimeRange.CResourceNo = this.cResourceNo;
        resAddTimeRange.resource = this;
        resAddTimeRange.CIsInfinityAbility = this.cIsInfinityAbility;
        ...
        if (resWorkTimeRange == null) {
            ...
            if (resWorkTimeRange.DBegTime <= resSpecTimeRange.DBegTime &&
                resWorkTimeRange.DEndTime > resSpecTimeRange.DBegTime) {
                resAddTimeRange.DBegTime = resWorkTimeRange.DEndTime;
                resAddTimeRange.DEndTime = resSpecTimeRange.DEndTime;
            }
            else if (resWorkTimeRange.DBegTime <= resSpecTimeRange.DBegTime &&
                resWorkTimeRange.DEndTime > resSpecTimeRange.DBegTime) {
                resAddTimeRange.DBegTime = resSpecTimeRange.DBegTime;
                resAddTimeRange.DEndTime = resSpecTimeRange.DEndTime;
            }
            else if (resWorkTimeRange.DEndTime < resSpecTimeRange.DBegTime ||
    ...
    addTimeRange(resLastTimeRange, resWorkTimeRange) {
        const resAddTimeRange = new type_1.ResTimeRange();
        resAddTimeRange.CResourceNo = this.cResourceNo;
        resAddTimeRange.resource = this;
        resAddTimeRange.CIsInfinityAbility = this.cIsInfinityAbility;
    ...
    deleteTimeRangeSub(resWorkTimeRange, resSpecTimeRange) {
        const resAddTimeRange = new type_1.TaskTimeRange();
        ...
        if (resAddTimeRange != null) {
            if (resWorkTimeRange.TaskTimeRangeList.length > 0) {
                const newTimeRange = resWorkTimeRange.GetNoWorkTaskTimeRange(resAddTimeRange.DBegTime, resAddTimeRange.DEndTime, false);
                ...
                resWorkTimeRange.TaskTimeRangeSplit(resWorkTimeRange.TaskTimeRangeList[0], newTimeRange);
            }
        }
    }
    getResSourceDayCapList() {
        this.ResSourceDayCapList = new Array(10);
        let resSourceDayCap = new type_1.ResSourceDayCap();
        let ldt_todayLast = new type_1.DateTime();
        const groupedResTimeRange = this.ResTimeRangeList.reduce((acc, resTimeRange) => {
        ...
        this.ResTimeRangeList.sort((p1, p2) => p1.DBegTime.getTime() - p2.DBegTime.getTime());
        for (const ResTimeRange1 of this.ResTimeRangeList) {
            if (ldt_todayLast.getTime() !== ResTimeRange1.dPeriodDay.getTime()) {
                resSourceDayCap = new type_1.ResSourceDayCap();
    ...
    schTaskFreezeInit(as_SchProductRouteRes, adCanBegDate, adCanEndDate) {
        let SchParam = this.SchParam;
        try {
            if (as_SchProductRouteRes.BScheduled == 1)
                return 0;
            let ResTimeRangeList1 = this.ResTimeRangeList.filter((p) => p.AvailableTime > 0 && p.DEndTime >= adCanBegDate);
            ResTimeRangeList1.sort((p1, p2) => p1.DBegTime.getTime() - p2.DBegTime.getTime());
            for (let i = 0; i < ResTimeRangeList1.length; i++) {
                if (ResTimeRangeList1[i].DBegTime > adCanEndDate)
                    break;
                ResTimeRangeList1[i].TimeSchTaskFreezeInit(as_SchProductRouteRes, adCanBegDate, adCanEndDate);
            }
            ...
            if (SchParam.APSDebug == '1') {
                const message2 = `3、排产顺序[${as_SchProductRouteRes.iSchSN}],资源编号[${as_SchProductRouteRes.cResourceNo}],物料编号[${as_SchProductRouteRes.cInvCode}], 座次号[${as_SchProductRouteRes.schProductRoute.schProduct.cSchSNType}],座次顺序[${as_SchProductRouteRes.schProductRoute.schProduct.iSchSN}],任务优先级[${as_SchProductRouteRes.iPriorityRes}],订单优先级[${as_SchProductRouteRes.schProductRoute.schProduct.iPriority}],工序[${as_SchProductRouteRes.iWoSeqID + as_SchProductRouteRes.cSeqNote.trim()}],排程批次[${as_SchProductRouteRes.iSchBatch}],工单号[${as_SchProductRouteRes.cWoNo}]`;
                SchParam.Debug(message2, 'SchProductRouteRes.SchTaskFreezeInit工序排产完成');
            }
        }
        catch (error) {
            throw new Error(`订单行号：${as_SchProductRouteRes.iSchSdID}资源正排计算时出错,位置Resource.ResSchTask！工序ID号：${as_SchProductRouteRes.iProcessProductID}\n\r ${error.message}`);
        }
        return 1;
    }
    schTaskSortInit(as_SchProductRouteRes, adCanBegDate, adCanEndDate) {
        let SchParam = this.SchParam;
        as_SchProductRouteRes.schProductRoute.ProcessSchTaskPre(false);
        let ai_workTime = 0;
        const ai_ResReqQty = as_SchProductRouteRes.iResReqQty - as_SchProductRouteRes.iActResReqQty;
        let iBatchCount = 0;
        if (as_SchProductRouteRes.cWorkType == '1') {
            if (ai_ResReqQty / as_SchProductRouteRes.iBatchQty <
        ...
        try {
            ...
            let li_Return = this.TestResSchTask(as_SchProductRouteRes, ai_workTimeTest, adCanBegDateTest, adCanBegDateTask, false, ai_ResPreTime, ai_CycTimeTol, dtBegDate, dtEndDate);
            if (li_Return < 0) {
                const cError = `订单行号：${as_SchProductRouteRes.iSchSdID} ,加工物料[${as_SchProductRouteRes.cInvCode}]在资源[${as_SchProductRouteRes.cResourceNo}]无法排下,任务号[${as_SchProductRouteRes.iProcessProductID}],单件产能[${as_SchProductRouteRes.iCapacity}],加工数量[${as_SchProductRouteRes.iResReqQty}],加工工时[${ai_workTimeTask / 3600}],未排工时[${ai_workTime / 3600}],最大可排时间[${adCanBegDateTest}],请检查工作日历或单件产能、计划数量太大!`;
            ...
            for (let i = 0; i < ResTimeRangeList1.length; i++) {
                if (ai_workTime == 0)
                    break;
                ResTimeRangeList1[i].TimeSchTask(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, true, ai_ResPreTime, ai_CycTimeTol, bFirtTime, ai_disWorkTime, false);
                if (bFirtTime) {
            ...
            if (ai_workTime > 0) {
                const cError = `订单行号：${as_SchProductRouteRes.iSchSdID} ,加工物料[${as_SchProductRouteRes.cInvCode}]在资源[${as_SchProductRouteRes.cResourceNo}]无法排下,任务号[${as_SchProductRouteRes.iProcessProductID}],单件产能[${as_SchProductRouteRes.iCapacity}],加工数量[${as_SchProductRouteRes.iResReqQty}],加工工时[${ai_workTimeTask / 3600}],未排工时[${ai_workTime / 3600}],最大可排时间[${ResTimeRangeList1[ResTimeRangeList1.length - 1].DEndTime}],请检查工作日历或单件产能、计划数量太大!`;
                throw new Error(cError);
                return -1;
            }
            else {
    ...
    resDispatchSch(iSchBatch) {
        let SchParam = this.SchParam;
        let SchProductRouteResPre = null;
        let ListSchProductRouteRes = [];
        let LastCanBegDate = new type_1.DateTime();
        try {
            if (iSchBatch == -100) {
                ListSchProductRouteRes = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo && p1.iResReqQty > 0);
                if (ListSchProductRouteRes.length > 0) {
                    ...
                    for (let i = 0; i < ListSchProductRouteRes.length; i++) {
                        ...
                        ListSchProductRouteRes[i].DispatchSchTask(ListSchProductRouteRes[i].iResReqQty, LastCanBegDate, SchProductRouteResPre);
                ...
                ListSchProductRouteRes = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                    p1.iResReqQty > 0 &&
    ...
    getResource(drResource) {
        ...
        if (this.cIsInfinityAbility == '')
            this.cIsInfinityAbility = '0';
        ...
        if (this.iOverEfficient == 0)
            this.iOverEfficient = 100;
        if (this.iLimitEfficient == 0)
            this.iLimitEfficient = 100;
        ...
        if (this.iMinWorkTime < 1)
            this.iMinWorkTime = SchParam.iTaskMinWorkTime;
        ...
        this.cDefine15 = new type_1.DateTime(drResource['cResDefine15']);
        this.cDefine16 = new type_1.DateTime(drResource['cResDefine16']);
        this.cDayPlanShowType = drResource['cDayPlanShowType'];
        this.dMaxExeDate = new type_1.DateTime(drResource['dMaxExeDate']);
    }
    GetTaskCanBegDate(schProductRouteRes, LastCanBegDate) {
        let dCanBegDate = LastCanBegDate;
        let dCanBegDateTemp = LastCanBegDate;
        schProductRouteRes.schProductRoute.SchProductRoutePreList.forEach((schProductRoutePre) => {
            ...
            if (dCanBegDateTemp > dCanBegDate)
                dCanBegDate = dCanBegDateTemp;
        });
        return dCanBegDate;
    }
    ResDispatchSchWo(iSchBatch) {
        let SchParam = this.SchParam;
        ...
        if (ListSchProductRouteRes.length > 0) {
            ...
            if (SchParam.cProChaType1Sort === '9') {
                if (this.cProChaType1Sort === '0') {
                    ListSchProductRouteRes.sort((p1, p2) => Comparer.default(p1.iPriorityRes, p2.iPriorityRes));
                }
                else if (this.cProChaType1Sort === '1') {
                    ListSchProductRouteRes.sort((p1, p2) => Comparer.default(p1.schProductRoute.schProductWorkItem.iWoPriorityRes, p2.schProductRoute.schProductWorkItem.iWoPriorityRes));
                }
                else if (this.cProChaType1Sort === '2') {
                    ListSchProductRouteRes.sort((p1, p2) => Comparer.default(p1.schProductRoute.schProductWorkItem.iPriority, p2.schProductRoute.schProductWorkItem.iPriority));
                }
                else if (this.cProChaType1Sort === '3') {
                    ListSchProductRouteRes.sort((p1, p2) => Comparer.default(p1.iSchSN, p2.iSchSN));
                }
                else if (this.cProChaType1Sort === '4') {
                    ListSchProductRouteRes.sort((p1, p2) => this.TaskComparer(p1, p2));
                }
                else {
                    ListSchProductRouteRes.sort((p1, p2) => Comparer.default(p1.dResBegDate, p2.dResBegDate));
                }
            }
            else {
                if (SchParam.cProChaType1Sort === '0') {
                    ...
                    ListSchProductRouteRes.sort((p1, p2) => Comparer.default(p1.schProductRoute.schProductWorkItem.dRequireDate, p2.schProductRoute.schProductWorkItem.dRequireDate));
                }
    ...
    ResDispatchSchTask(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_ResPreTime, ai_CycTimeTol, bReCalWorkTime = true) {
        let SchParam = this.SchParam;
        let ai_workTimeTask = ai_workTime;
        let adCanBegDateTask = adCanBegDate;
        ...
        try {
            let adCanBegDateTest = adCanBegDate;
            let ai_workTimeTest = ai_workTime;
            SchParam.ldtBeginDate = new type_1.DateTime();
            let li_Return = this.TestResSchTask(as_SchProductRouteRes, ai_workTimeTest, adCanBegDateTest, adCanBegDateTask, false, ai_ResPreTime, ai_CycTimeTol, dtBegDate, dtEndDate, bReCalWorkTime);
            if (li_Return < 0) {
                let cError = `订单行号：${as_SchProductRouteRes.iSchSdID} ,加工物料[${as_SchProductRouteRes.cInvCode}]在资源[${as_SchProductRouteRes.cResourceNo}]无法排下,任务号[${as_SchProductRouteRes.iProcessProductID}],单件产能[${as_SchProductRouteRes.iCapacity}],加工数量[${as_SchProductRouteRes.iResReqQty}],加工工时[${ai_workTimeTask / 3600}],未排工时[${ai_workTime / 3600}],最大可排时间[${adCanBegDateTest}],请检查工作日历或单件产能、计划数量太大!`;
                throw new Error(cError);
            }
            adCanBegDate = adCanBegDateTask;
            let ResTimeRangeList1 = this.ResTimeRangeList.filter((p) => p.AvailableTime > 0 && p.DEndTime > adCanBegDate);
            ResTimeRangeList1.sort((p1, p2) => Comparer.default(p1.DBegTime, p2.DBegTime));
            let bFirtTime = true;
            for (let i = 0; i < ResTimeRangeList1.length; i++) {
                if (bFirtTime &&
                    ...
                    continue;
                let ldtBeginDateRessource = new type_1.DateTime();
                ResTimeRangeList1[i].TimeSchTask(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, true, ai_ResPreTime, ai_CycTimeTol, bFirtTime, ai_disWorkTime, bReCalWorkTime);
                if (bFirtTime) {
                    dtBegDate = ResTimeRangeList1[i].DBegTime;
                    as_SchProductRouteRes.dResLeanBegDate = ResTimeRangeList1[i].DBegTime;
                }
            ...
            throw new Error(`订单行号：${as_SchProductRouteRes.iSchSdID}资源正排计算时出错,位置Resource.ResDispatchSchTask！工序ID号：${as_SchProductRouteRes.iProcessProductID} \n\r ${error.message}`);
        }
        return 1;
    }
    ResSchTask(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_ResPreTime, ai_CycTimeTol, bReCalWorkTime = true, as_SchProductRouteResPre = null) {
        let SchParam = this.SchParam;
        let ai_workTimeTask = ai_workTime;
        let adCanBegDateTask = adCanBegDate;
        let ldtBeginDate = new type_1.DateTime();
        ...
        try {
            ...
            let li_Return = this.TestResSchTask(as_SchProductRouteRes, ai_workTimeTest, adCanBegDateTest, adCanBegDateTask, false, ai_ResPreTime, ai_CycTimeTol, dtBegDate, dtEndDate, bReCalWorkTime, true, as_SchProductRouteResPre);
            if (li_Return < 0) {
                let cError = `订单行号：${as_SchProductRouteRes.iSchSdID} ,加工物料[${as_SchProductRouteRes.cInvCode}]在资源[${as_SchProductRouteRes.cResourceNo}]无法排下,任务号[${as_SchProductRouteRes.iProcessProductID}],单件产能[${as_SchProductRouteRes.iCapacity}],加工数量[${as_SchProductRouteRes.iResReqQty}],加工工时[${ai_workTimeTask / 3600}],未排工时[${ai_workTime / 3600}],最大可排时间[${adCanBegDateTest}],请检查工作日历或单件产能、计划数量太大!`;
                throw new Error(cError);
            }
    ...
    ResSchTaskRev(as_SchProductRouteRes, ai_workTime, adCanBegDate) {
        let Comparer = this.Comparer;
        let SchParam = this.SchParam;
        let ai_workTimeTask = ai_workTime;
        let adCanBegDateTask = adCanBegDate;
        ...
        try {
            let adCanBegDateTest = adCanBegDate;
            let ai_workTimeTest = ai_workTime;
            let li_Return = this.TestResSchTask(as_SchProductRouteRes, ai_workTimeTest, adCanBegDateTest, adCanBegDateTask, true, ai_ResPreTime, ai_CycTimeTol, dtBegDate, dtEndDate);
            if (li_Return < 0) {
                let cError = `订单行号：${as_SchProductRouteRes.iSchSdID} ,加工物料[${as_SchProductRouteRes.cInvCode}]在资源[${as_SchProductRouteRes.cResourceNo}]无法排下,任务号[${as_SchProductRouteRes.iProcessProductID}],单件产能[${as_SchProductRouteRes.iCapacity}],加工数量[${as_SchProductRouteRes.iResReqQty}],加工工时[${ai_workTimeTask / 3600}],未排工时[${ai_workTime / 3600}],最大可排时间[${adCanBegDateTest}],请检查工作日历或单件产能、计划数量太大!`;
                throw new Error(cError);
            }
            adCanBegDate = adCanBegDateTask;
            let ResTimeRangeList1 = this.ResTimeRangeList.filter((p) => p.AvailableTime > 0 && p.DBegTime <= adCanBegDate);
            ResTimeRangeList1.sort((p1, p2) => Comparer.default(p2.DBegTime, p1.DBegTime));
            let bFirtTime = true;
            for (let i = 0; i < ResTimeRangeList1.length; i++) {
                if (ai_workTime == 0)
                    break;
                if (as_SchProductRouteRes.schProductRoute.schProduct.cSchType != '2')
                    ResTimeRangeList1[i].TimeSchTaskRev(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, true, bFirtTime);
                else
                    ResTimeRangeList1[i].TimeSchTaskRevInfinite(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, true, bFirtTime);
            }
            if (ai_workTime > 0) {
                let cError = `订单行号：${as_SchProductRouteRes.iSchSdID} ,加工物料[${as_SchProductRouteRes.cInvCode}]在资源[${as_SchProductRouteRes.cResourceNo}]无法排下,任务号[${as_SchProductRouteRes.iProcessProductID}],单件产能[${as_SchProductRouteRes.iCapacity}],加工数量[${as_SchProductRouteRes.iResReqQty}],加工工时[${ai_workTimeTask / 3600}],未排工时[${ai_workTime / 3600}],最大可排时间[${ResTimeRangeList1[ResTimeRangeList1.length - 1].DEndTime}],请检查工作日历或单件产能、计划数量太大!`;
                throw new Error(cError);
            ...
            throw new Error(`订单行号：${as_SchProductRouteRes.iSchSdID}资源倒排计算时出错,位置Resource.ResSchTaskRev！工序ID号：${as_SchProductRouteRes.iProcessProductID} \n\r ${error.message}`);
        }
        return 1;
    }
    TestResSchTask(as_SchProductRouteRes, ai_workTime, adCanBegDate, adCanBegDateTask, bSchRev, ai_ResPreTime, ai_CycTimeTol, dtBegDate, dtEndDate, bShowTips = true, bReCalWorkTime = true, as_SchProductRouteResPre = null) {
        let SchParam = this.SchParam;
        let ai_workTimeTask = ai_workTime;
        let ai_disWorkTime = ai_workTime;
        ...
        let Comparer = this.Comparer;
        try {
            let ResTimeRangeList1 = [];
            if (!bSchRev) {
                ResTimeRangeList1 = this.ResTimeRangeList.filter((p) => p.DEndTime > adCanBegDateTask2);
                ...
                for (let i = 0; i < ResTimeRangeList1.length; i++) {
                    ...
                    if (ResTimeRangeList1[i].AvailableTime <= 0 &&
                    ...
                    try {
                        SchParam.ldtBeginDate = new type_1.DateTime();
                        ResTimeRangeList1[i].TimeSchTask(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, false, ai_ResPreTime, ai_CycTimeTol, bFirtTime, ai_disWorkTime, bReCalWorkTime, resTimeRangeNext, as_SchProductRouteResPre);
                    }
                    catch (error) {
                        throw new Error(`时间段排程出错,订单行号：${as_SchProductRouteRes.iSchSdID}资源正排计算时出错,位置SchProductRoute.ProcessSchTask！工序ID号：${as_SchProductRouteRes.iProcessProductID} \n\r ${error.message}`);
                    }
                    if (bFirtTime) {
                ...
                ResTimeRangeList1 = this.ResTimeRangeList.filter((p) => p.DBegTime < adCanBegDateTask2);
                ResTimeRangeList1.sort((p1, p2) => Comparer.default(p2.DBegTime, p1.DBegTime));
                let bFirtTime = true;
                for (let i = 0; i < ResTimeRangeList1.length; i++) {
                    if (ai_workTime == 0)
                        break;
                    if (as_SchProductRouteRes.schProductRoute.schProduct.cSchType != '2')
                        ResTimeRangeList1[i].TimeSchTaskRev(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, false, bFirtTime);
                    else
                        ResTimeRangeList1[i].TimeSchTaskRevInfinite(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, false, bFirtTime);
                    if (bFirtTime) {
                        dtEndDate = adCanBegDate;
                        bFirtTime = false;
                        dtBegDate = adCanBegDate;
                        ...
                        if (ResTimeRangeList1[i + 1].WorkTimeRangeList.length > 0 &&
                            ResTimeRangeList1[i + 1].NotWorkTime < ai_workTime) {
                            bFirtTime = true;
                            ai_workTime = ai_workTimeTask;
                            adCanBegDate =
                                ResTimeRangeList1[i + 1].WorkTimeRangeList[ResTimeRangeList1[i + 1].WorkTimeRangeList.length - 1].DEndTime;
                        }
                    }
                }
                dtBegDate = adCanBegDate;
    ...
    TestResSchTaskNew(as_SchProductRouteRes, ai_workTime, adCanBegDate, adCanBegDateTask, bSchRev, ai_ResPreTime, ai_CycTimeTol, dtBegDate, dtEndDate, bShowTips = true, bReCalWorkTime = true, as_SchProductRouteResPre = null) {
        let Comparer = this.Comparer;
        let ai_workTimeTask = ai_workTime;
        let ai_disWorkTime = ai_workTime;
        let adCanBegDateTask2 = adCanBegDateTask;
        ...
        try {
            ...
            if (!bSchRev) {
                ...
                for (let i = 0; i < ResTimeRangeList1.length; i++) {
                    if (ai_workTime == 0)
                        break;
                    ResTimeRangeList1[i].TimeSchTask(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, false, ai_ResPreTime, ai_CycTimeTol, bFirtTime, ai_disWorkTime, bReCalWorkTime);
                    if (bFirtTime) {
                        dtBegDate = adCanBegDate;
                        as_SchProductRouteRes.dResLeanBegDate = adCanBegDate;
                    }
    ...
    //   return 1
    // }
    GetEarlyStartDate(adStartDate, bSchRev) {
        let ListReturn = [];
        let Comparer = this.Comparer;
        let iCanSchTime = 0;
        let ResTimeRangeBeg = null;
        let ResTimeRangeEnd = null;
        let dtBegDate;
        let TaskTimeRangeBeg = null;
        let TaskTimeRangeEnd = null;
        let dtEndDate = adStartDate;
        if (!bSchRev) {
            ListReturn = this.ResTimeRangeList.filter((p) => p.DEndTime > adStartDate && p.AvailableTime > 0);
            ...
            ListReturn = this.ResTimeRangeList.filter((p) => p.DBegTime < adStartDate && p.AvailableTime > 0);
    ...
    GetSchStartDate(as_SchProductRouteRes, ai_workTime, adStartDate, bSchRev, dtEndDate) {
        let SchParam = this.SchParam;
        let ListReturn = new Array(10);
    ...
    GetChangeTime(as_SchProductRouteRes, ai_workTime, adStartDate, iCycTimeTol, bSchdule, as_SchProductRouteResPre = null) {
        let Comparer = this.Comparer;
        let SchParam = this.SchParam;
        let iPreTime = 0;
        if (this.cNeedChanged === '1' &&
            as_SchProductRouteRes.cWoNo !== '' &&
            as_SchProductRouteRes.schProductRoute.schProduct.cType === 'PSH')
            return 0;
        if (as_SchProductRouteResPre === null) {
            let ListSchProductRouteResAll = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                p1.iResReqQty > 0 &&
                p1.BScheduled === 1 &&
                p1.dResBegDate <= adStartDate);
            ListSchProductRouteResAll.sort((p1, p2) => Comparer.default(p2.dResBegDate, p1.dResBegDate));
        ...
        if (SchParam.ResProcessCharCount > 0) {
            iPreTime = this.GetChangeTime(as_SchProductRouteRes, ai_workTime, 
            //@ts-ignore
            as_SchProductRouteResPre, iCycTimeTol, bSchdule);
        }
        if (iPreTime > 0) {
        ...
        if (as_SchProductRouteResPre !== null) {
            if (as_SchProductRouteRes.schProductRoute.cWorkItemNo !==
    ...
    //   return iPreTime
    // }
    KeyResSchTask(iCount = 1) {
        let SchParam = this.SchParam;
        let Comparer = this.Comparer;
        this.getNotSchTask();
        if (this.schProductRouteResList.length < 1)
            return 1;
        this.schData.SchProductList.sort((p1, p2) => Comparer.default(p1.iSchPriority, p2.iSchPriority));
        let iSchPriority = this.schData.SchProductList[this.schData.SchProductList.length - 1].iSchPriority;
        if (iSchPriority < 0)
            iSchPriority = 0;
        let as_SchProductRouteResLast = null;
        this.schProductRouteResList.forEach((schProductRouteRes) => {
            if (schProductRouteRes.iProcessProductID === SchParam.iProcessProductID &&
    ...
    KeyResSchTaskPre() {
        let Comparer = this.Comparer;
        let schProductRouteResList = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo && p1.iSchBatch === this.iSchBatch);
    ...
    KeyResSchTaskSingle(as_iTurmsTime, as_SchProductRouteResLast) {
        let Comparer = this.Comparer;
        let SchParam = this.SchParam;
        this.schData.SchProductList.sort((p1, p2) => Comparer.default(p1.iSchPriority, p2.iSchPriority));
        let iSchPriority = this.schData.SchProductList[this.schData.SchProductList.length - 1].iSchPriority;
        ...
        if (this.iTurnsType === '1') {
            ...
            if (this.iTurnsTime <= 0 && as_SchProductRouteResLast !== null) {
                ...
                if (as_SchProductRouteResLast.resource.cResourceNo !== this.cResourceNo &&
            ...
            while (iTolWorkTime < as_iTurmsTime) {
                let schProductRouteRes = this.schProductRouteResList.find((p1) => p1.cResourceNo === this.cResourceNo &&
                    p1.schProductRoute.BScheduled === 0);
                if (schProductRouteRes === null) {
                    SchParam.Debug(`关键资源${this.cResourceNo}轮换排产,轮换批次[${this.iBatch.toString()}],总工时[${(iTolWorkTime / 60).toString()}],所有任务已排完.`, '关键资源轮换生产');
                    this.iBatch++;
                    return -1;
                }
                schProductRouteRes.schProductRoute.ProcessSchTaskPre(false);
                if (SchParam.bSchKeyBySN === '1' &&
            ...
            this.iBatch++;
            SchParam.Debug(`关键资源${this.cResourceNo}轮换排产,轮换批次[${this.iBatch.toString()}],总工时[${(iTolWorkTime / 60).toString()}]`, '关键资源轮换生产');
        }
            ...
            while (iTolWorks < as_iTurmsTime || this.iTurnsTime === 0) {
                ...
                iTolWorks += 1;
                as_SchProductRouteResLast = schProductRouteRes;
                if (this.cBatch2WorkTime < 10)
                    this.cBatch2WorkTime = 10;
                if (this.iTurnsTime === 0 &&
    ...
    KeyResBatch() {
        ...
        let as_SchProductRouteResLast = new type_1.SchProductRouteRes();
        try {
            let schProductRouteResListNoTest = schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                p1.iWoSeqID === this.iBatchWoSeqID &&
                p1.iSchBatch === this.iSchBatch);
            let schProductRouteResListNoSeq = schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                p1.schProductRoute.BScheduled === 0 &&
                p1.iSchBatch === this.iSchBatch);
            let schProductRouteResListNo = schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                p1.schProductRoute.BScheduled === 0 &&
                p1.iWoSeqID === this.iBatchWoSeqID &&
                p1.iSchBatch === this.iSchBatch);
            if (schProductRouteResListNo.length < 1)
                return -1;
            schProductRouteResListNo.forEach((schProductRouteRes) => {
                schProductRouteRes.schProductRoute.ProcessSchTaskPre(false);
                schProductRouteRes.schProductRoute.GetRouteEarlyBegDate();
                schProductRouteRes.dEarliestStartTime =
                    schProductRouteRes.schProductRoute.dEarlyBegDate;
            });
            schProductRouteResListNo.sort((p1, p2) => this.TaskComparer(p1, p2));
            if (this.iSchBatch === 1) {
                let schProductRouteResListFirst = schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                    p1.iWoSeqID === this.iBatchWoSeqID &&
                    p1.iSchBatch === this.iSchBatch &&
                    p1.iBatch === 1);
                if (schProductRouteResListFirst.length > 0) {
                    this.KeyResBatchSchTaskSubFirst(1, as_SchProductRouteResLast, schProductRouteResListFirst);
                }
            }
            if (this.cBatch2Filter === '' &&
                ...
                this.cBatch4Filter === '' &&
                this.cBatch5Filter === '') {
                this.KeyResBatchSchTask(this.cBatch1Filter, this.iBatchWoSeqID, true, this.iTurnsTime, schProductRouteResListNo, as_SchProductRouteResLast);
            }
            else {
                let bExist = true;
                let iBatchCount = 1;
                while (bExist) {
                    if (this.cBatch1Filter !== '')
                        this.KeyResBatchSchTask(this.cBatch1Filter, this.iBatchWoSeqID, false, this.cBatch1WorkTime * 60, schProductRouteResListNo, as_SchProductRouteResLast);
                    if (this.cBatch2Filter !== '')
                        this.KeyResBatchSchTask(this.cBatch2Filter, this.iBatchWoSeqID, false, this.cBatch2WorkTime * 60, schProductRouteResListNo, as_SchProductRouteResLast);
                    if (this.cBatch3Filter !== '')
                        this.KeyResBatchSchTask(this.cBatch3Filter, this.iBatchWoSeqID, false, this.cBatch3WorkTime * 60, schProductRouteResListNo, as_SchProductRouteResLast);
                    if (this.cBatch4Filter !== '')
                        this.KeyResBatchSchTask(this.cBatch4Filter, this.iBatchWoSeqID, false, this.cBatch4WorkTime * 60, schProductRouteResListNo, as_SchProductRouteResLast);
                    if (this.cBatch5Filter !== '')
                        this.KeyResBatchSchTask(this.cBatch5Filter, this.iBatchWoSeqID, false, this.cBatch5WorkTime * 60, schProductRouteResListNo, as_SchProductRouteResLast);
                    iBatchCount++;
                    let schProductRouteResListNo2 = schProductRouteResListNo.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                        p1.schProductRoute.BScheduled === 0);
                    ...
                    if (iBatchCount > 20) {
                        this.KeyResBatchSchTask('', this.iBatchWoSeqID, true, 0, schProductRouteResListNo, as_SchProductRouteResLast);
                        break;
                    }
                }
                this.KeyResBatchSchTask('', this.iBatchWoSeqID, true, 0, schProductRouteResListNo, as_SchProductRouteResLast);
    ...
    KeyResBatchSchTask(cBatchFilter, iBatchWoSeqID, bSchAll, as_iTurmsTime, as_schProductRouteResListNo, as_SchProductRouteResLast) {
        let schProductRouteResListNo = new Array(10);
        let schProductRouteResListLast = new Array(10);
        let Comparer = this.Comparer;
        if (cBatchFilter !== '') {
            schProductRouteResListNo = as_schProductRouteResListNo.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                p1.schProductRoute.BScheduled === 0 &&
        ...
        if (schProductRouteResListNo.length < 1)
            return -1;
        schProductRouteResListNo.sort((p1, p2) => Comparer.Default.Compare(p1.schProductRoute.dEarlyBegDate, p2.schProductRoute.dEarlyBegDate));
        let iTolWorkTime = 0;
        if (as_iTurmsTime < 1)
            as_iTurmsTime = this.iTurnsTime;
        if (as_iTurmsTime < 1)
        ...
        for (let i = 0; i < schProductRouteResListNo.length; i++) {
            ...
            if (iTolWorkTime > as_iTurmsTime) {
                as_SchProductRouteResLast = schProductRouteRes;
                this.SchParam.Debug(`资源编号${this.cResourceNo}，分批${this.iBatch},分批条件${cBatchFilter}，分批工时${(as_iTurmsTime / 60).toString()}，累计工时${(iTolWorkTime / 60).toString()}，总任务数${schProductRouteResListNo.length.toString()}`, '资源分批');
                this.KeyResBatchSchTaskSub(this.iBatch, as_SchProductRouteResLast, as_schProductRouteResListNo, cBatchFilter);
                iTolWorkTime = 0;
                this.iBatch++;
                if (!bSchAll)
                    return this.iBatch;
            ...
            if (i === schProductRouteResListNo.length - 1) {
                as_SchProductRouteResLast = schProductRouteRes;
                this.SchParam.Debug(`资源编号${this.cResourceNo}最后一批，分批${this.iBatch},分批条件${cBatchFilter}，分批工时${(as_iTurmsTime / 60).toString()}，累计工时${(iTolWorkTime / 60).toString()}，总任务数${schProductRouteResListNo.length.toString()}`, '资源分批');
                this.KeyResBatchSchTaskSub(this.iBatch, as_SchProductRouteResLast, as_schProductRouteResListNo, cBatchFilter);
                this.iBatch++;
                if (!bSchAll)
                    return this.iBatch;
    ...
    KeyResBatchSchTaskSub(as_iBatch, as_SchProductRouteResLast, as_schProductRouteResListNo, cBatchFilter) {
        let Comparer = this.Comparer;
        let schProductRouteResListBatch = as_schProductRouteResListNo.filter((p1) => p1.cResourceNo === this.cResourceNo &&
            p1.schProductRoute.BScheduled === 0 &&
            p1.iWoSeqID === this.iBatchWoSeqID &&
            p1.iBatch === as_iBatch &&
            p1.iSchBatch === this.iSchBatch);
        if (schProductRouteResListBatch.length < 1)
            return -1;
        let dEndDateTemp = this.SchParam.dtToday;
        if (this.dBatchEndDate > this.SchParam.dtToday) {
            dEndDateTemp = this.dBatchEndDate;
        }
        schProductRouteResListBatch.forEach((schProductRouteRes) => {
        ...
        this.dBatchBegDate = new type_1.DateTime(this.dBatchBegDate.getTime() + this.iPreStocks * 60000);
        this.dBatchBegDate = new type_1.DateTime(this.dBatchBegDate.getTime() + this.iPostStocks * 60000);
        schProductRouteResListBatch.forEach((schProductRouteRes) => {
            schProductRouteRes.schProductRoute.dEarlyBegDate = this.dBatchBegDate;
        });
        schProductRouteResListBatch.forEach((schProductRouteRes) => {
            schProductRouteRes.schProductRoute.ProcessSchTask();
            let schProductRouteResListWorkItem = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                p1.schProductRoute.BScheduled === 0 &&
                p1.iWoSeqID > this.iBatchWoSeqID &&
                p1.cInvCode === schProductRouteRes.cInvCode &&
                p1.iSchSdID === schProductRouteRes.iSchSdID);
            schProductRouteResListWorkItem.forEach((schProductRouteResNext) => {
                schProductRouteResNext.iBatch = as_iBatch;
                schProductRouteResNext.iSchSN = schProductRouteRes.iSchSN;
            });
            as_SchProductRouteResLast = schProductRouteRes;
        });
        let schProductRouteResListNext = new Array(10);
        let schProductRouteResListNextAdd = new Array(10);
        for (let iNextSeqID = this.iBatchWoSeqID + 1; iNextSeqID < 90; iNextSeqID++) {
            schProductRouteResListNext = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                p1.schProductRoute.BScheduled === 0 &&
                p1.iSchBatch === this.iSchBatch &&
                p1.iBatch === as_iBatch &&
                p1.iWoSeqID === iNextSeqID);
            if (schProductRouteResListNext.length > 0) {
                schProductRouteResListNext.sort((p1, p2) => Comparer.Default.Compare(p1.iSchSN, p2.iSchSN));
                schProductRouteResListNext.forEach((ResNext) => {
            ...
            if (cBatchFilter !== '') {
                schProductRouteResListNextAdd = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
                    p1.schProductRoute.BScheduled === 0 &&
                    cBatchFilter.indexOf(p1.FResChaValue1ID) >= 0 &&
                    p1.FResChaValue1ID !== '' &&
                    p1.iWoSeqID === iNextSeqID &&
            ...
            if (schProductRouteResListNextAdd.length > 0) {
                schProductRouteResListNextAdd.sort((p1, p2) => Comparer.Default.Compare(p1.iSchSN, p2.iSchSN));
                schProductRouteResListNextAdd.forEach((ResNext) => {
        ...
        let schProductRouteResListSched = this.schData.SchProductRouteResList.filter((p1) => p1.cResourceNo === this.cResourceNo &&
            p1.schProductRoute.BScheduled === 1 &&
            p1.iSchBatch === this.iSchBatch &&
            p1.iBatch === as_iBatch &&
            p1.schProductRoute.cStatus !== '4');
        schProductRouteResListSched.sort((p1, p2) => Comparer.Default.Compare(p1.dResBegDate, p2.dResBegDate));
        if (schProductRouteResListSched.length > 0) {
            this.dBatchBegDate =
                schProductRouteResListSched[schProductRouteResListSched.length - 1].dResEndDate;
        }
        return 1;
    }
    KeyResBatchSchTaskSubFirst(as_iBatch, as_SchProductRouteResLast, as_schProductRouteResListNo) {
        let Comparer = this.Comparer;
        let dEndDateTemp = this.SchParam.dtToday;
        this.dBatchBegDate = dEndDateTemp;
        let schProductRouteResListNext = new Array(10);
    ...
    ResClearTask(aSchProductRoute) {
        aSchProductRoute.SchProductRouteResList.forEach((as_SchProductRouteRes) => {
        ...
        if (this.iSchBatch !== 1) {
            if (this.cIsKey === '1' && this.cProChaType1Sort === '1') {
                if (Comparer.Default.Compare(p1.iResourceID, p2.iResourceID) === 0) {
                    if (p1.resChaValue1 != null &&
                        p2.resChaValue1 != null &&
                        Comparer.Default.Compare(p1.resChaValue1.FSchSN, p2.resChaValue1.FSchSN) === 0) {
                        return 1;
                    }
                        ...
                        return Comparer.Default.Compare(p1.resChaValue1.FSchSN, p2.resChaValue1.FSchSN);
                    }
                }
                else {
                ...
                if (p1.resChaValue1 != null &&
                    p2.resChaValue1 != null &&
                    Comparer.Default.Compare(p1.resChaValue1.FSchSN, p2.resChaValue1.FSchSN) === 0) {
                    if (p1.resChaValue2 != null &&
                        p2.resChaValue2 != null &&
                        Comparer.Default.Compare(p1.resChaValue2.FSchSN, p2.resChaValue2.FSchSN) === 0) {
                        if (p1.resChaValue3 != null &&
                            p2.resChaValue3 != null &&
                            Comparer.Default.Compare(p1.resChaValue3.FSchSN, p2.resChaValue3.FSchSN) === 0) {
                            if (p1.resChaValue4 != null &&
                                p2.resChaValue4 != null &&
                                Comparer.Default.Compare(p1.resChaValue4.FSchSN, p2.resChaValue4.FSchSN) === 0) {
                                if (p1.resChaValue5 != null &&
                                    p2.resChaValue5 != null &&
                                    Comparer.Default.Compare(p1.resChaValue5.FSchSN, p2.resChaValue5.FSchSN) === 0) {
                                    if (p1.resChaValue6 != null &&
                                        p2.resChaValue6 != null &&
                                        Comparer.Default.Compare(p1.resChaValue6.FSchSN, p2.resChaValue6.FSchSN) === 0) {
                                        return 1;
                                    }
                                    else {
                                        if (p1.resChaValue6 == null || p2.resChaValue6 == null) {
                                            ...
                                            return Comparer.Default.Compare(p1.resChaValue6.FSchSN, p2.resChaValue6.FSchSN);
                                        }
                                    }
                                }
                                else {
                                    if (p1.resChaValue5 == null || p2.resChaValue5 == null) {
                                        ...
                                        return Comparer.Default.Compare(p1.resChaValue5.FSchSN, p2.resChaValue5.FSchSN);
                                    }
                                }
                            }
                            else {
                                if (p1.resChaValue4 == null || p2.resChaValue4 == null) {
                                    ...
                                    return Comparer.Default.Compare(p1.resChaValue4.FSchSN, p2.resChaValue4.FSchSN);
                                }
                            }
                        }
                        else {
                            if (p1.resChaValue3 == null || p2.resChaValue3 == null) {
                                ...
                                return Comparer.Default.Compare(p1.resChaValue3.FSchSN, p2.resChaValue3.FSchSN);
                            }
                        }
                    }
                    else {
                        if (p1.resChaValue2 == null || p2.resChaValue2 == null) {
                            ...
                            return Comparer.Default.Compare(p1.resChaValue2.FSchSN, p2.resChaValue2.FSchSN);
                        }
                    }
                }
                else {
            ...
            if (Comparer.Default.Compare(p1.schProductRoute.schProduct.cMiNo, p2.schProductRoute.schProduct.cMiNo) === 0) {
                ...
                return Comparer.Default.Compare(p1.schProductRoute.schProduct.cMiNo, p2.schProductRoute.schProduct.cMiNo);
            }
        }
        return 1;
    }
    ResTaskComparer(p1, p2) {
        ...
        if (this.SchParam.cProChaType1Sort === '5') {
            ...
            if (p1.schProductRoute.schProductWorkItem == null ||
                p2.schProductRoute.schProductWorkItem == null) {
                if (Comparer.Default.Compare(p1.schProductRoute.schProduct.iPriority, p2.schProductRoute.schProduct.iPriority) === 0) {
                    return Comparer.Default.Compare(p1.dResBegDate, p2.dResBegDate);
                }
                else {
                    return Comparer.Default.Compare(p1.schProductRoute.schProduct.iPriority, p2.schProductRoute.schProduct.iPriority);
                }
            }
            else {
                return Comparer.Default.Compare(p1.schProductRoute.schProductWorkItem.dRequireDate, p2.schProductRoute.schProductWorkItem.dRequireDate);
            }
        }
            ...
            if (p1.schProductRoute.schProductWorkItem == null ||
                ...
                if (Comparer.Default.Compare(p1.schProductRoute.schProductWorkItem.iPriority, p2.schProductRoute.schProductWorkItem.iPriority) === 0) {
                    return Comparer.Default.Compare(p1.dResBegDate, p2.dResBegDate);
                }
                else {
                    return Comparer.Default.Compare(p1.schProductRoute.schProductWorkItem.iPriority, p2.schProductRoute.schProductWorkItem.iPriority);
                }
            }
        }
    ...
    getTaskTimeRangeList(orderASC = true) {
        const listTaskTimeRangeAll = [];
        ...
        if (orderASC) {
            // Ascending order
            listTaskTimeRangeAll.sort((p1, p2) => p1.dBegTime.getTime() - p2.dBegTime.getTime());
        }
        else {
            // Descending order
            listTaskTimeRangeAll.sort((p1, p2) => p2.dBegTime.getTime() - p1.dBegTime.getTime());
        }
