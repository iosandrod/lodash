...
exports.ResTimeRange = void 0;
const Base_1 = require("./Base");
const type_1 = require("./type");
class ResTimeRange extends Base_1.Base {
    constructor(as_ResourceNo, adBegTime, adEndTime) {
        super();
        this.schData = null; // 所有排程数据
        this.cResourceNo = ''; // 对应资源ID号,要设置
        this.cIsInfinityAbility = '0'; // 0 产能有限，1 产能无限
        this.resource = null; // 时段对应的资源 有值
        this.dBegTime = new type_1.DateTime(); // 时间段开始时间
        this.dEndTime = new type_1.DateTime(); // 时间段结束时间
        this.holdingTime = 0; // 时段总长, dEndTime - dBegTime,单位为秒
        ...
        this.dPeriodDay = new type_1.DateTime(); // 时段所属日期
        this.FShiftType = ''; // 时段所属班次 白班、夜班、中班等
        this.taskTimeRangeList = new Array(10);
        this.WorkTimeRangeList = new Array(10);
        this.ResTimeRangePre = null; // 前资源时间段
        this.ResTimeRangePost = null; // 后资源时间段
        this.iSchSdID = -1; // 记录更新、新增时间段的任务ID
        this.iProcessProductID = -1;
        this.iResProcessID = -1;
        this.iSchSNMax = -1;
        this.resSourceDayCap = null; // 资源日产能限制,2023-10-05 新增加，每个时间段排程完成时，设置当天日产能已排工时。
        if (as_ResourceNo) {
    ...
    TimeSchTaskFreezeInit(as_SchProductRouteRes, adCanBegDate, adCanEndDate) {
        let SchParam = this.SchParam;
        let bSchdule = true; // 正式排产
        let NoTimeRangeList = this.GetAvailableTimeRangeList(adCanBegDate, false, bSchdule, as_SchProductRouteRes.iResRationHour + as_SchProductRouteRes.iResPreTime, false);
        try {
            for (let i = 0; i < NoTimeRangeList.length; i++) {
                let NoTimeRange = NoTimeRangeList[i];
                if (NoTimeRange.DBegTime >= adCanEndDate)
                    break; // 时间段大于待排结束时间段，退出
                if (NoTimeRange.AvailableTime < SchParam.PeriodLeftTime) {
                    continue;
                }
                let taskTimeRange1 = new type_1.TaskTimeRange();
                ...
                if (NoTimeRange.dBegTime >= adCanBegDate) {
                    if (adCanEndDate > NoTimeRange.DEndTime) {
                        ...
                        taskTimeRange1.AllottedTime = Math.floor((taskTimeRange1.DEndTime.getTime() -
                            taskTimeRange1.DBegTime.getTime()) /
                            1000);
                    }
                }
                else if (NoTimeRange.dBegTime < adCanBegDate &&
                ...
                if (taskTimeRange1.DBegTime < new type_1.DateTime('2011-01-01')) {
                ...
                if (this.cIsInfinityAbility != '1' &&
                    ...
                    throw new Error(`出错位置1：排程时段分配时间大于时段总工时!TimeSchTask.TaskTimeRangeSplit！时段开始时间:${NoTimeRange.dBegTime.toString()}时段结束时间:${NoTimeRange.dEndTime.toString()}任务开始时间:${taskTimeRange1.dBegTime.toString()}任务结束时间:${taskTimeRange1.dEndTime.toString()}`);
                    return -1;
                }
                ...
                if (!this.CheckResTimeRange() && this.cIsInfinityAbility != '1') {
                    throw new Error('出错位置：排程时段检查出错TimeSchTask.CheckResTimeRange！');
                    return -1;
                }
            }
        }
    ...
    TimeSchTaskSortInit(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, bSchdule, ai_ResPreTime, ai_CycTimeTol, bFirtTime) {
        let SchParam = this.SchParam;
        ...
        let NoTimeRangeList = this.GetAvailableTimeRangeList(adCanBegDate, false, bSchdule, ai_workTime + ai_ResPreTime, false);
        try {
            for (let i = 0; i < NoTimeRangeList.length; i++) {
                if (ai_workTime <= 0)
                    break;
                ...
                if (bFirtTime) {
                    ...
                    ai_CycTimeTol = 0; // 设为0
                    ai_ResPreTime = this.resource.GetChangeTime(as_SchProductRouteRes, ai_workTime, NoTimeRange.DBegTime, ai_CycTimeTol, bSchdule);
                    if (ai_ResPreTime > 0) {
                        let K = 0;
                    }
                ...
                if (NoTimeRange.dBegTime >= adCanBegDate) {
                    if (ai_workTime > NoTimeRange.AvailableTime) {
                        ...
                        // 部分占用
                        taskTimeRange1.DBegTime = NoTimeRange.DBegTime;
                        taskTimeRange1.DEndTime = new type_1.DateTime(NoTimeRange.DBegTime.getTime() + ai_workTime * 1000);
                    ...
                    if (NoTimeRange.DEndTime > adCanBegDate) {
                        let lTimeSpan = NoTimeRange.DEndTime.getTime() - adCanBegDate.getTime();
                        ...
                        if (ai_workTime > iAvailableTime) {
                            ...
                            // 部分占用,排完
                            taskTimeRange1.DBegTime = adCanBegDate;
                            taskTimeRange1.DEndTime = new type_1.DateTime(adCanBegDate.getTime() + ai_workTime * 1000);
                            taskTimeRange1.AllottedTime = ai_workTime;
                            ai_workTime = 0; // 剩余待分配工作时间为0
                        }
                    }
    ...
    TimeSchTask(as_SchProductRouteRes, ai_workTime, adCanBegDate, ai_workTimeTask, adCanBegDateTask, bSchdule, ai_ResPreTime, ai_CycTimeTol, bFirtTime, ai_DisWorkTime, bReCalWorkTime = true, resTimeRangeNext = null, as_SchProductRouteResPre = null) {
        let SchParam = this.SchParam;
        let taskallottedTime = 0; // 任务在本时间段内 总安排时间
        let ai_workTimeOld = ai_workTime; // 用于记录ai_workTime值
        let iDayDis = 1; // 考虑学习曲线每天折扣
        let dtiDayDis = new type_1.DateTime(adCanBegDate.getTime() - 86400000); // 学习曲线日期
        ...
        let NoTimeRangeList = this.GetAvailableTimeRangeList(adCanBegDate, false, bSchdule, ai_workTime + ai_ResPreTime, true);
        if ((as_SchProductRouteRes.iProcessProductID == SchParam.iProcessProductID &&
            ...
            // 调试断点1 SchProduct
            message = `3.4.2、TimeSchTask 排产顺序[${as_SchProductRouteRes.iSchSN}],计划ID[${as_SchProductRouteRes.iSchSdID}],任务ID[${as_SchProductRouteRes.iProcessProductID}],资源编号[${as_SchProductRouteRes.cResourceNo}],开始排产时间[${new type_1.DateTime()}],完成排产时间[${type_1.SchData.GetDateDiffString(ldtBeginDate, new type_1.DateTime(), 'ms')}]`;
            ...
            for (let i = 0; i < NoTimeRangeList.length; i++) {
                ...
                if (bFirtTime) {
                    ...
                    if (bReCalWorkTime) {
                        ...
                        ai_CycTimeTol = 0; // 设为0
                        if (this.resource.iSchBatch <= 1 &&
                            as_SchProductRouteRes.iActResReqQty > 0)
                            ai_ResPreTime = 0;
                        else {
                            ai_ResPreTime = this.resource.GetChangeTime(as_SchProductRouteRes, ai_workTime, NoTimeRange.DBegTime, ai_CycTimeTol, bSchdule, as_SchProductRouteResPre);
                        }
                        if (ai_ResPreTime > 0) {
                            let K = 0;
                        }
                ...
                if (bSchdule == false) {
                    ...
                    if (NoTimeRange.taskTimeRangePost != null &&
                ...
                if ((as_SchProductRouteRes.iProcessProductID ==
                    ...
                    (as_SchProductRouteRes.iProcessProductID == 193864 &&
                        as_SchProductRouteRes.schProductRoute.iSchSdID == 1070)) {
                    // 调试断点1 SchProduct
                    message = `3.4.3、TimeSchTask 排产顺序[${as_SchProductRouteRes.iSchSN}],计划ID[${as_SchProductRouteRes.iSchSdID}],任务ID[${as_SchProductRouteRes.iProcessProductID}],资源编号[${as_SchProductRouteRes.cResourceNo}],开始排产时间[${new type_1.DateTime()}],完成排产时间[${type_1.SchData.GetDateDiffString(ldtBeginDate, new type_1.DateTime(), 'ms')}]`;
                    SchParam.Debug(message, '资源运算');
                    ldtBeginDate = new type_1.DateTime();
                }
                if (NoTimeRange.AvailableTime < SchParam.PeriodLeftTime &&
                ...
                if ((as_SchProductRouteRes.iProcessProductID ==
                    ...
                    (as_SchProductRouteRes.iProcessProductID == 193864 &&
                        as_SchProductRouteRes.schProductRoute.iSchSdID == 1070)) {
                    // 调试断点1 SchProduct
                    message = `3.4.4、TimeSchTask 排产顺序[${as_SchProductRouteRes.iSchSN}],计划ID[${as_SchProductRouteRes.iSchSdID}],任务ID[${as_SchProductRouteRes.iProcessProductID}],资源编号[${as_SchProductRouteRes.cResourceNo}],开始排产时间[${new type_1.DateTime()}],完成排产时间[${type_1.SchData.GetDateDiffString(ldtBeginDate, new type_1.DateTime(), 'ms')}]`;
                    SchParam.Debug(message, '资源运算');
                    ldtBeginDate = new type_1.DateTime();
                }
                if (bSchdule) {
                    // 正式排程
                    if (this.cResourceNo == '3.04.24' &&
                        as_SchProductRouteRes.iSchSdID == SchParam.iSchSdID) {
                        // 调试断点1 SchProduct
                        let ii = 1;
                    }
                    if ((as_SchProductRouteRes.iSchSdID == SchParam.iSchSdID &&
                        as_SchProductRouteRes.iProcessProductID ==
                            SchParam.iProcessProductID) ||
                ...
                if (bSchdule) {
                    // 正式排程
                    if (!this.CheckResTimeRange() && this.cIsInfinityAbility != '1') {
                        let Errormessage = `检查时段数据出错CheckResTimeRange,排产顺序[${as_SchProductRouteRes.iSchSN}],计划ID[${as_SchProductRouteRes.iSchSdID}],任务ID[${as_SchProductRouteRes.iProcessProductID}],资源编号[${as_SchProductRouteRes.cResourceNo}],时段开始时间[${this.dBegTime}],时段结束时间[${this.dEndTime}],时段总工时[${this.holdingTime}],分配工时[${this.allottedTime}],空闲工时[${this.notWorkTime}],差异工时[${this.AvailableTime - this.notWorkTime}]`;
        ...
        if ((as_SchProductRouteRes.iProcessProductID == SchParam.iProcessProductID &&
            ...
            (as_SchProductRouteRes.iProcessProductID == 193864 &&
                as_SchProductRouteRes.schProductRoute.iSchSdID == 1070)) {
            // 调试断点1 SchProduct
            message = `3.4.5、TimeSchTask 排产顺序[${as_SchProductRouteRes.iSchSN}],计划ID[${as_SchProductRouteRes.iSchSdID}],任务ID[${as_SchProductRouteRes.iProcessProductID}],资源编号[${as_SchProductRouteRes.cResourceNo}],开始排产时间[${new type_1.DateTime()}],完成排产时间[${type_1.SchData.GetDateDiffString(ldtBeginDate, new type_1.DateTime(), 'ms')}]`;
            SchParam.Debug(message, '资源运算');
            ldtBeginDate = new type_1.DateTime();
        }
        return ai_workTime; // 剩下未排时间
    }
    TimeSchTaskRev(as_SchProductRouteRes, ai_workTime, adCanEndDate, ai_workTimeTask, adCanBegDateTask, bSchdule, bFirtTime) {
        let SchParam = this.SchParam;
        let taskallottedTime = 0; // 任务在本时间段内 总安排时间
        let ai_workTimeOld = 0; // 用于记录ai_workTime值，当空闲时间段太下时，如10分钟，则该时间段不排，ai_workTime值还原。
        let NoTimeRangeList = this.GetAvailableTimeRangeList(adCanEndDate, true, bSchdule, ai_workTime, true);
        if ((as_SchProductRouteRes.iSchSdID == SchParam.iSchSdID &&
            as_SchProductRouteRes.iProcessProductID ==
                SchParam.iProcessProductID) ||
            as_SchProductRouteRes.iResourceAbilityID == SchParam.iProcessProductID) {
        ...
        if (!this.CheckResTimeRange() && this.cIsInfinityAbility != '1') {
            throw new Error('出错位置：倒排排程时段检查出错TimeSchTask.TimeSchTaskRev！');
            return -1;
        }
        try {
            for (let i = 0; i < NoTimeRangeList.length; i++) {
                ...
                if (bSchdule == false) {
                    // 没有排完
                    if (NoTimeRange.taskTimeRangePre != null &&
                        ai_workTime > NoTimeRange.AvailableTime) {
                        bFirtTime = true; // 是否第一个排产时间段
                        ai_workTime = ai_workTimeTask; // 返回原值
                ...
                if (NoTimeRange.DEndTime <= adCanEndDate) {
                    if (ai_workTime > NoTimeRange.AvailableTime) {
                        ...
                        taskTimeRange1.DBegTime = new type_1.DateTime(NoTimeRange.DEndTime.getTime() - ai_workTime * 1000);
                        taskTimeRange1.AllottedTime = ai_workTime;
                        ai_workTime = 0; // 剩余待分配工作时间为0
                    }
                }
                    ...
                    if (NoTimeRange.DBegTime < adCanEndDate) {
                        let lTimeSpan = adCanEndDate.getTime() - NoTimeRange.DBegTime.getTime();
                        let iAvailableTime = Math.floor(lTimeSpan / 1000);
                        if (ai_workTime > iAvailableTime) {
                            taskTimeRange1.DBegTime = NoTimeRange.DBegTime;
                            taskTimeRange1.DEndTime = adCanEndDate;
                            ...
                            // 部分占用,排完
                            taskTimeRange1.DBegTime = new type_1.DateTime(adCanEndDate.getTime() - ai_workTime * 1000);
                            taskTimeRange1.DEndTime = adCanEndDate;
                            taskTimeRange1.AllottedTime = ai_workTime;
                            ai_workTime = 0; // 剩余待分配工作时间为0
                        }
                ...
                if (taskTimeRange1.DBegTime < NoTimeRange.DBegTime &&
                    taskTimeRange1.DBegTime > NoTimeRange.DEndTime) {
                    throw new Error('数据异常，生成新任务开始时间小于可用时间段,且结束时间大于可用时间段!');
                    return -1;
                }
                if (bSchdule) {
                    // 正式排程
    ...
    TimeSchTaskRevInfinite(as_SchProductRouteRes, ai_workTime, adCanEndDate, ai_workTimeTask, adCanBegDateTask, bSchdule, bFirtTime) {
        let SchParam = this.SchParam;
        let taskallottedTime = 0; // 任务在本时间段内 总安排时间
        let ai_workTimeOld = 0; // 用于记录ai_workTime值，当空闲时间段太下时，如10分钟，则该时间段不排，ai_workTime值还原。
        let NoTimeRangeList = this.GetAvailableTimeRangeList(adCanEndDate, true, bSchdule, ai_workTime, false);
        if ((as_SchProductRouteRes.iSchSdID == SchParam.iSchSdID &&
            as_SchProductRouteRes.iProcessProductID ==
                SchParam.iProcessProductID) ||
            as_SchProductRouteRes.iResourceAbilityID == SchParam.iProcessProductID) {
    ...
    CheckResTimeRange() {
        ...
        if (Math.abs(this.AvailableTime - this.notWorkTime) > 5) {
            ...
            this.ThowErrText(`检查分配时间${this.allottedTime} > 空闲时间${this.notWorkTime}`); // 小数点，有误差
            return false;
        }
        let liNotWorkTime = 0;
        ...
        if (Math.abs(liNotWorkTime - this.notWorkTime) > 5) {
            this.ThowErrText(`检查空闲时间${this.notWorkTime}与明细汇总${liNotWorkTime}不一致`); // 小数点，有误差
            return false;
        }
        if (Math.abs(this.allottedTime + this.notWorkTime - this.holdingTime) > 5) {
            this.ThowErrText(`检查分配时间${this.allottedTime} + 空闲时间${this.notWorkTime} 大于 时间段总长${this.holdingTime}`); // 小数点，有误差
            return false;
        }
        return true;
    }
    ThowErrText(ErrText) {
        throw new Error('出错位置：排程时段检查出错TimeSchTask.ThowErrText！' + ErrText);
        return false; // 小数点，有误差
    }
    GetAvailableTimeRange(adCanBegDate, bSchRev = false) {
        let lTaskTimeRangeFind = null;
        let TaskTimeRangeTemp = [];
        if (!this.CheckResTimeRange() && this.cIsInfinityAbility != '1') {
            throw new Error('出错位置：排程时段检查出错TimeSchTask.TaskTimeRangeSplit！');
            return null;
        }
        if (this.CIsInfinityAbility == '1') {
            // 产能无限，整个时段都可用
            TaskTimeRangeTemp = this.TaskTimeRangeList.filter((p2) => p2.cTaskType == 0);
            ...
            if (bSchRev) {
                // 倒排
                TaskTimeRangeTemp = this.TaskTimeRangeList.filter((p2) => ((p2.DBegTime <= adCanBegDate && p2.DEndTime > adCanBegDate) ||
    ...
    GetAvailableTimeRangeList(adCanBegDate, bSchRev, bSchdule = true, ai_workTime = 0, bIncludeWorkTime = true) {
        let Comparer = this.Comparer;
        if (bSchdule && bIncludeWorkTime)
            bIncludeWorkTime = false;
        let lTaskTimeRangeList = [];
        if (this.CIsInfinityAbility == '1') {
            lTaskTimeRangeList = this.TaskTimeRangeList.filter((p2) => p2.cTaskType == 0);
            lTaskTimeRangeList.sort((p1, p2) => Comparer.default(p1.DBegTime, p2.DBegTime));
        }
        else {
            if (bSchRev) {
                if (bSchdule) {
                    lTaskTimeRangeList = this.TaskTimeRangeList.filter((p2) => p2.DBegTime <= adCanBegDate && p2.cTaskType == 0);
                }
                else {
                    lTaskTimeRangeList = this.TaskTimeRangeList.filter((p2) => p2.DBegTime <= adCanBegDate && p2.cTaskType == 0);
                    if (bIncludeWorkTime)
                ...
                lTaskTimeRangeList.sort((p1, p2) => Comparer.default(p2.DBegTime, p1.DBegTime));
            }
            else {
                if (bSchdule) {
                    lTaskTimeRangeList = this.TaskTimeRangeList.filter((p2) => p2.DEndTime > adCanBegDate && p2.cTaskType == 0);
                }
                else {
                    lTaskTimeRangeList = this.TaskTimeRangeList.filter((p2) => p2.DEndTime > adCanBegDate && p2.cTaskType == 0);
    ...
    GetAvailableTime(adCanBegDate, bSchRev = false) {
        ...
        if (this.CIsInfinityAbility == '1') {
            ...
            if (bSchRev) {
                ...
                return new type_1.DateTime();
            }
        }
    }
    GetResTimeRange(adCanBegDate, as_Type = '1') {
        const lResTimeRangeList = this.resource.ResTimeRangeList.filter((p2) => p2.DBegTime <= adCanBegDate && p2.DEndTime > adCanBegDate);
        if (lResTimeRangeList.length > 0)
            return lResTimeRangeList[0];
        else
            return null;
    }
    TaskTimeRangeSplit(aToltalTaskRange, aNewTaskRange) {
        let SchParam = this.SchParam;
        let NoTaskTime1 = null;
        let NoTaskTime2 = null;
        if (this.CIsInfinityAbility == '1') {
            this.WorkTimeRangeList.push(aNewTaskRange);
        }
        else {
            try {
                if (aNewTaskRange.iProcessProductID == SchParam.iProcessProductID &&
                ...
                if (this.AllottedTime + aNewTaskRange.AllottedTime > this.HoldingTime) {
                    throw new Error(`出错位置1：排程时段分配时间大于时段总工时!TimeSchTask.TaskTimeRangeSplit！时段开始时间:${aToltalTaskRange.DBegTime.toString()}时段结束时间:${aToltalTaskRange.DEndTime.toString()}任务开始时间:${aNewTaskRange.DBegTime.toString()}任务结束时间:${aNewTaskRange.DEndTime.toString()}`);
                    return -1;
                }
                if (aToltalTaskRange.DBegTime == aNewTaskRange.DBegTime &&
                    ...
                    NoTaskTime1 = this.GetNoWorkTaskTimeRange(aNewTaskRange.DEndTime, aToltalTaskRange.DEndTime);
                    ...
                    NoTaskTime1 = this.GetNoWorkTaskTimeRange(aToltalTaskRange.DBegTime, aNewTaskRange.DBegTime);
                    this.ModifyResTimeRange(aNewTaskRange, aToltalTaskRange, NoTaskTime1);
                    aToltalTaskRange.taskTimeRangePost = aNewTaskRange;
                }
                else {
                    NoTaskTime1 = this.GetNoWorkTaskTimeRange(aToltalTaskRange.DBegTime, aNewTaskRange.DBegTime);
                    NoTaskTime2 = this.GetNoWorkTaskTimeRange(aNewTaskRange.DEndTime, aToltalTaskRange.DEndTime);
                    this.ModifyResTimeRange(aNewTaskRange, aToltalTaskRange, NoTaskTime1, NoTaskTime2);
                    aToltalTaskRange.taskTimeRangePost = aNewTaskRange;
                    if (NoTaskTime2)
                        NoTaskTime2.taskTimeRangePre = aNewTaskRange;
                }
                if (this.CIsInfinityAbility != '1' && !this.CheckResTimeRange()) {
                    throw new Error(`出错位置：排程时段检查出错TimeSchTask.CheckResTimeRange！时段开始时间:${aToltalTaskRange.DBegTime.toString()}时段结束时间:${aToltalTaskRange.DEndTime.toString()}任务开始时间:${aNewTaskRange.DBegTime.toString()}任务结束时间:${aNewTaskRange.DEndTime.toString()}`);
                    return -1;
                }
            }
            catch (exp) {
    ...
    GetNoWorkTaskTimeRange(dBegDate, dEndDate, bCreate = false) {
        const NoTaskTime = new type_1.TaskTimeRange();
        ...
        NoTaskTime.HoldingTime = Math.floor((NoTaskTime.DEndTime.getTime() - NoTaskTime.DBegTime.getTime()) / 1000);
    ...
    CopyTaskTimeRange(aOldTaskRange, aNewTaskRange) {
        let SchParam = this.SchParam;
    ...
    MegTaskTimeRangeAll() {
        let Comparer = this.Comparer;
        if (this.TaskTimeRangeList.length <= 1)
            return 1;
        let TaskTimeRangePre = null;
        let TaskTimeRangeNew = null;
        this.TaskTimeRangeList.sort((p1, p2) => Comparer.default(p1.DBegTime, p2.DBegTime));
        const iCount = this.TaskTimeRangeList.length;
        let allottedTime = 0;
        for (let i = iCount - 1; i >= 0; i--) {
            TaskTimeRangeNew = this.TaskTimeRangeList[i];
            if (TaskTimeRangePre != null &&
                TaskTimeRangePre.cTaskType == 0 &&
                TaskTimeRangeNew.cTaskType == 0 &&
                TaskTimeRangeNew.DEndTime == TaskTimeRangePre.DBegTime) {
                TaskTimeRangeNew = this.MegTaskTimeRange(TaskTimeRangePre, TaskTimeRangeNew);
                if (TaskTimeRangeNew == null) {
                    throw new Error('检验任务合并时间段出错,位置ReTimeRange.MegTaskTimeRangeAll');
                    return -1;
                }
            }
    ...
    MegTaskTimeRange(TaskTimeRangeLast, TaskTimeRangeNew) {
        let SchParam = this.SchParam;
        if (TaskTimeRangeLast.cTaskType != 0)
            return null;
        if (TaskTimeRangeNew.cTaskType != 0)
            return null;
        const NoWorkTaskTime = this.GetNoWorkTaskTimeRange(TaskTimeRangeNew.DBegTime, TaskTimeRangeLast.DEndTime, false);
        ...
        if (Math.abs(TaskTimeRangeLast.HoldingTime +
            TaskTimeRangeNew.HoldingTime -
            NoWorkTaskTime.HoldingTime) > 5) {
            ...
            SchParam.Debug(message, '资源运算');
            throw new Error('检验任务拆分后时间段出错,位置ReTimeRange.MegTaskTimeRange!' + message);
            return null;
        }
        if (!this.CheckResTimeRange()) {
            const message = `2、原时间段长[${TaskTimeRangeLast.HoldingTime}],计划ID[${TaskTimeRangeLast.iSchSdID}],任务ID[${TaskTimeRangeLast.iProcessProductID}],资源编号[${TaskTimeRangeLast.cResourceNo}],新时间段长[${TaskTimeRangeNew.HoldingTime}],未分配时间段长[${NoWorkTaskTime.HoldingTime}]`;
            SchParam.Debug(message, '资源运算');
            throw new Error('清除任务时合并空闲时间段出错,位置ReTimeRange.MegTaskTimeRange！' +
                message);
            return null;
        }
    ...
    CheckTaskOverlap(as_SchProductRouteRes, dt_ResDate, bSchRev = false) {
        if (this.CheckCurTimeTaskOverlap(as_SchProductRouteRes, dt_ResDate, dt_ResDate, bSchRev) < 0)
            return -1;
        if (bSchRev == false) {
            if (this.CheckNextTimeTaskOverlap(as_SchProductRouteRes, this.DEndTime, dt_ResDate, bSchRev) < 0)
                return -1;
        }
        else {
            if (this.CheckNextTimeTaskOverlap(as_SchProductRouteRes, this.DBegTime, dt_ResDate, bSchRev) < 0)
                return -1;
        }
        return 1;
    }
    CheckCurTimeTaskOverlap(as_SchProductRouteRes, dt_ResDate, adCanBegDate, bSchRev = false) {
        if (this.TaskTimeRangeList.length < 1)
            return 1;
        if (this.CIsInfinityAbility == '1')
            return 1;
        try {
            const TaskTimeRange2 = this.TaskTimeRangeList.find((p1) => p1.iSchSdID == as_SchProductRouteRes.iSchSdID &&
            ...
            if (TaskTimeRange2 != null) {
                ...
                if (bSchRev == false) {
                    ...
                    const TaskTimeRangeList1 = this.resource
                        ...
                        .filter((p1) => p1.DBegTime >= ldtResEndDate &&
                        p1.CResourceNo == this.cResourceNo);
                    if (TaskTimeRangeList1.length < 1)
                        return -1;
                    const TaskTimeRange3 = TaskTimeRangeList1[0];
                    if (TaskTimeRange3 != null &&
                    ...
                    const TaskTimeRangeList1 = this.resource
                        ...
                        .filter((p1) => p1.DEndTime <= ldtResEndDate &&
                        p1.CResourceNo == this.cResourceNo);
                    if (TaskTimeRangeList1.length < 1)
                        return -1;
                    const TaskTimeRange3 = TaskTimeRangeList1[0];
            ...
            throw new Error('检验任务是否重叠出错,位置ReTimeRange.CheckCurTimeTaskOverlap！工序ID号：' +
                as_SchProductRouteRes.iProcessProductID +
                '\n\r ' +
                error.message);
            return -1;
    ...
    CheckNextTimeTaskOverlap(as_SchProductRouteRes, dt_ResDate, adCanBegDate, bSchRev = false) {
        const Comparer = this.Comparer;
        const SchData = this.schData;
        const SchParam = this.SchParam;
        let ResTimeRange1 = null;
        if (this.CIsInfinityAbility == '1')
            return 1;
        try {
            if (bSchRev == false) {
                const ResTimeRangeList = this.resource.ResTimeRangeList.filter((p1) => p1.DBegTime >= dt_ResDate && p1.CResourceNo == this.cResourceNo);
                ResTimeRangeList.sort((p1, p2) => Comparer.default(p1.DBegTime, p2.DBegTime));
                ...
                const ResTimeRangeList = this.resource.ResTimeRangeList.filter((p1) => p1.DEndTime <= dt_ResDate && p1.CResourceNo == this.cResourceNo);
                ResTimeRangeList.sort((p1, p2) => Comparer.default(p1.DBegTime, p2.DBegTime));
                if (ResTimeRangeList.length > 0) {
            ...
            throw new Error('检验任务是否重叠出错,位置ReTimeRange.CheckNextTimeTaskOverlap！工序ID号：' +
                as_SchProductRouteRes.iProcessProductID +
                '\n\r ' +
                error.message);
            return -1;
    ...
    ModifyResTimeRange(aNewTaskRange, oldTaskTimeRange, NoTaskTime1, NoTaskTime2 = null) {
        if (!this.CheckResTimeRange() && this.CIsInfinityAbility != '1') {
            throw new Error('出错位置：排程时段检查出错TimeSchTask.TaskTimeRangeSplit！');
            return -1;
        }
        aNewTaskRange.addWorkTimeRange(this);
        if (NoTaskTime1 == null) {
    ...
    set DBegTime(value) {
        if (value < new type_1.DateTime(2000, 1, 1)) {
            throw new Error(`资源编号${this.cResourceNo}时间段开始日期${value.toString()}不能小于2000-01-01日,开始时间${this.dBegTime.toDateString()},结束时间${this.dEndTime.toDateString()}`);
        }
        this.dBegTime = value;
    ...
    set DEndTime(value) {
        if (value < type_1.DateTime.fromObject({ year: 2000, month: 1, day: 1 })) {
            throw new Error(`资源编号${this.cResourceNo}时间段结束日期${value.toISO()}不能小于2000-01-01日,开始时间${this.dBegTime.toLocaleString()},结束时间${this.dEndTime.toLocaleString()}`);
        }
        if (value <= this.dBegTime) {
            throw new Error(`资源编号${this.cResourceNo}时间段结束日期${value.toISO()}不能小于时段开始时间,开始时间${this.dBegTime.toLocaleString()},结束时间${this.dEndTime.toLocaleString()}`);
        }
        this.dEndTime = value;
        this.holdingTime = this.HoldingTime;
    }
...
exports.ResTimeRange = ResTimeRange;
