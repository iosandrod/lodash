...
class Scheduling extends type_1.Base {
    constructor() {
        super();
        this.myTimer = null;
    }
    ...
    SchMainRun(as_SchType = '1') {
        let SchParam = this.SchParam;
        if (this.schData.dtToday > new type_1.DateTime('2024-11-20')) {
        ...
        if (as_SchType === '1') {
            ...
            for (let iSchBatch = -10; iSchBatch < 20; iSchBatch++) {
                let schProductList = this.schData.SchProductList.filter((p1) => p1.iSchBatch === iSchBatch);
                if (schProductList.length < 1)
                    continue;
        ...
        if (this.SchRunPost() < 1)
            return -1;
        this.myTimer = null;
        return 1;
    }
    DispatchSchRun(as_iSchBatch) {
        // let SchParam = this.SchParam
        let cResourceNo = '';
        ...
        try {
            let KeyResourceListTemp1 = this.schData.ResourceList.filter((p1) => p1.cIsKey === '1');
            KeyResourceListTemp1.sort((p1, p2) => Comparer.numeric(p1.iKeySchSN, p2.iKeySchSN));
            ...
            let KeyResourceListTemp3 = this.schData.ResourceList.filter((p1) => p1.cIsKey !== '1');
            KeyResourceListTemp3.sort((p1, p2) => Comparer.numeric(p1.iKeySchSN, p2.iKeySchSN));
    ...
    SchRunBatch(iSchBatch) {
        let SchParam = this.SchParam;
        let cResourceNo = '';
        if (SchParam.cSchRunType === '1') {
            if (this.SchRunBatchBySN(iSchBatch) < 0)
                return -1;
            return 1;
        }
        else {
    ...
    SchRunBatchBySN(iSchBatch) {
        let SchParam = this.SchParam;
        let Comparer = this.Comparer;
        let schProductList = this.schData.SchProductList.filter((p1) => p1.iSchBatch === iSchBatch && p1.bScheduled === 0);
        ...
        let schProductRouteTempList = this.schData.SchProductRouteList.filter((p1) => p1.iSchBatch === iSchBatch && p1.BScheduled === 0);
        if (schProductRouteTempList.length < 1)
            return 1;
        schProductRouteTempList.sort((p1, p2) => Comparer.date(p1.dBegDate, p2.dBegDate));
        schProductRouteTempList.forEach((schProductRoute) => {
            schProductRoute.ProcessSchTaskPre();
        });
        return 1;
    }
    SchRunBatchByWoSN(as_iSchBatch) {
        let SchParam = this.SchParam;
        let Comparer = this.Comparer;
        for (let iSchBatch = -10; iSchBatch < 20; iSchBatch++) {
            let schProductWorkItemList = this.schData.SchProductWorkItemList.filter((p1) => p1.iSchBatch === iSchBatch && p1.bScheduled === 0);
            if (schProductWorkItemList.length > 0) {
                if (SchParam.cProChaType1Sort === '1') {
                    schProductWorkItemList.sort((p1, p2) => Comparer.date(p1.dRequireDate, p2.dRequireDate));
                }
                else if (SchParam.cProChaType1Sort === '2') {
                    schProductWorkItemList.sort((p1, p2) => Comparer.numeric(p1.iPriority, p2.iPriority));
                }
                else if (SchParam.cProChaType1Sort === '3') {
                    schProductWorkItemList.sort((p1, p2) => Comparer.numeric(p1.iSchSN, p2.iSchSN));
                }
                else {
                    schProductWorkItemList.sort((p1, p2) => Comparer.date(p1.dBegDate, p2.dBegDate));
                }
                try {
    ...
    PerProductSchTask(as_SchProduct) {
        let SchParam = this.SchParam;
        if (as_SchProduct == null)
            return;
        let lSchProduct = as_SchProduct;
        if (lSchProduct.cSchType === '0') {
            lSchProduct.ProductSchTask();
            if (lSchProduct.bSet === '1' &&
                SchParam.SetMinDelayTime > 0 &&
    ...
    PerBatchEnd() {
        let maxWorkerThreads, workerThreads, portThreads;
        while (true) {
    ...
    SchRunBatchByFreeze(iSchBatch) {
        ...
        if (schProductList.length < 1)
            return 0;
        try {
            let schProductRouteResList = this.schData.SchProductRouteResList.filter((p1) => p1.BScheduled === 0 &&
                p1.iSchBatch === iSchBatch &&
                p1.iResReqQty > 0);
            if (schProductRouteResList.length < 1)
                return 1;
            schProductRouteResList.sort((p1, p2) => Comparer.numeric(p1.cDefine34, p2.cDefine34));
            ...
            schProductRouteResList.forEach((schProductRouteRes) => {
                ...
                if (schProductRouteRes.resource.iTurnsTime !== 0 &&
    ...
    SchRunPre() {
        let SchParam = this.SchParam;
        let iWorkTime = 0;
        let resSchProductRouteResListNull = this.schData.SchProductRouteResList.filter((p) => p.schProductRoute == null);
        if (resSchProductRouteResListNull.length > 0) {
            resSchProductRouteResListNull.forEach((schProductRouteRes) => {
                this.schData.SchProductRouteResList.splice(this.schData.SchProductRouteResList.indexOf(schProductRouteRes), 1);
            });
        }
        let resSchProductRouteResListComp = this.schData.SchProductRouteResList.filter((p) => p.schProductRoute.cStatus === '4' && p.cWoNo !== '');
    ...
    SchRunDataPre() {
        ...
        this.schData.SchProductList.forEach((schProduc) => {
            let dr = this.schData.dtItem.filter((item) => item.cInvCode === schProduc.cInvCode);
            if (dr.length > 0) {
    ...
    SchRun() {
        let Comparer = this.Comparer;
        this.schData.SchProductList.sort((p1, p2) => Comparer.numeric(p1.iPriority, p2.iPriority));
