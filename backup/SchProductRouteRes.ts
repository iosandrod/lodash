...
export class SchProductRouteRes {
  ...
  set BScheduled(value: number) {
    ...
    if (
      ...
      SchParam.dtResLastSchTime = new Date()
    ...
    try {
      ...
      if (
        ...
        message = `2、排产顺序[${this.iSchSN}],计划ID[${
          ...
          this.cResourceNo
        }],开始排产时间[${new Date()}],运算时间[${SchData.GetDateDiffString(
          ldtBeginDate,
          new Date(),
          'ms',
        )}]`
      ...
      let ldtBeginDateRes = new Date()
      if (this.cSeqNote === '折弯') {
        ldtBeginDateRes = new Date()
      }
      try {
      ...
      const iWaitTime = new Date().getTime() - ldtBeginDateRes.getTime()
      if (this.cSeqNote === '折弯') {
      } else {
      }
      if (
  ...
  TaskSchTask(ai_ResReqQty: number, adCanBegDate: Date): number {
    let fWorkTime = 0
    let iWorkTime = 0
    let iBatch = 0
  ...
  TestResSchTask(ai_ResReqQty: number, adCanBegDate: Date): number {
    let fWorkTime = 0
    let iWorkTime = 0
    let iBatch = 0
    if (this.bScheduled === 1) return 0
