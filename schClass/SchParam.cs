...
{    
    public static class SchParam 
    {
        ...
        public static int ResProcessCharCount = 4;	 //资源使用工艺特征总数	4	定义资源档案中，每个资源的最大使用工艺特数

        public static int TaskSelectRes = 1;           //任务选择机台原则	1	1、订单交期优先，选择最早可用机台 (默认)   2、换产时间最小优先 
        public static int PeriodLeftTime = 20;     //	每个工作时间段剩余多少分种不安排任务	20	每个工作时间段剩余多少分钟不排新任务。如下午下班前20分钟，不再安排新任务
        public static int TaskSchNotBreak = 1;      //	排程任务不能中断	1	排程任务不能中断，必须连续排产。资源会出现一些小的时间间隙，但不能完全排下，则该时间段不能利用。  1、排程任务不能中断  0、排程任务可以中断
        ...
        public static string cTechSchType = "0";      //工艺段排产方式  0	普通排产 1	注塑排产 2	装配排产 3	表面处理 4	委外排产
        ...
        {
            //1 使用; 0 不使用  2 不使用APS系统，直接发放生成生产任务单 3 使用APS， 接ERP系统生产任务单排产
            UseAPS =GetParam("UseAPS");

            //资源使用工艺特征总数	4	定义资源档案中，每个资源的最大使用工艺特数	
            ResProcessCharCount = Convert.ToInt16(GetParam("ResProcessCharCount"));

            //任务选择机台原则    //任务选择机台原则	1	1、订单交期优先，选择最早可用机台 (默认)   2、换产时间最小优先 
            TaskSelectRes = Convert.ToInt16(GetParam("TaskSelectRes"));

            //PeriodLeftTime 每个工作时间段剩余多少分种不安排任务 每个工作时间段剩余多少分种不安排任务	20	每个工作时间段剩余多少分钟不排新任务。如下午下班前5分钟，不再安排新任务
            PeriodLeftTime = Convert.ToInt16(GetParam("PeriodLeftTime")) * 60;

            //排程任务不能中断 1	排程任务不能中断，必须连续排产。资源会出现一些小的时间间隙，但不能完全排下，则该时间段不能利用。  1、排程任务不能中断  0、排程任务可以中断

            TaskSchNotBreak = Convert.ToInt16(GetParam("TaskSchNotBreak"));

            //  需配套生产最大相差时间    需配套生产物料，最大相差时间（小时），如8小时 ,越大计算次数越小，越快

            DiffMaxTime = Convert.ToInt16(GetParam("DiffMaxTime"));

            //  配套最少延期时间          需配套生产物料，最后一部件生产完工后，最少延期多长时间，装配开始,越大，缺料情况越少。 
            SetMinDelayTime = Convert.ToInt16(GetParam("SetMinDelayTime"));

            //  所有资源利用率%          //所有资源利用率%          最大为100%，正常不到100%,资源本身定义的效益生效
            ...
            //后工序可开工时间为前工序开工后多长时间(分钟) 120
            NextSeqBegTime = Convert.ToInt32(GetParam("NextSeqBegTime"));

            //重排已下达任务开工时间  已下达任务重排开工时间,关键工序生产顺序不变(按原来计划开工时间顺序序), 1 重排 0 不重排
            ...
            ////考虑前工序完工时间约束 1 考虑前工序完工时间约束; 0 不考虑,资源任务会排连续 ，参数不用，正式排产时，选冻结方式，不考虑前工序约束
            //PreSeqEndDate = GetParam("PreSeqEndDate");
            ...
            try
            {
                //Assembly ass = Assembly.LoadFrom(System.IO.Directory.GetCurrentDirectory() + "\\APS.exe");  //当前系统运行路径

                //Logger = ass.GetType("APS.Common.Logger", true, true);

                //objLogger = Activator.CreateInstance(Logger);
        ...
        //public static Type Logger;                   //日志处理对象
        ...
        public static void Debug(String message,String form ,String Event = "")
        {
            ...
            try
            {
                ...
                //Logger.InvokeMember("Debug", BindingFlags.InvokeMethod, null, objLogger, parameters);
                //objLogger.Debug(message, form, Event);
            }
            catch (Exception exp)
            {
        ...
        public static void Error(String message, String form, String Event = "")
        {
            ...
            try
            {
                ...
                //Logger.InvokeMember("Error", BindingFlags.InvokeMethod, null, objLogger, parameters);
               // objLogger.Debug(message, form, Event);
            }
            catch (Exception exp)
            {
