...
import {
  ...
  TechInfo,
  WorkCenter,
  SchParam as _SchParam,
} from './type'
import { Base } from './Base'
type DLShowProcess = (iProgress: number, message: string) => void
export class SchInterface extends Base {
  //@ts-ignore
  public ldtBegDate: DateTime
  //@ts-ignore
  public sqlPro: any = new APSRun.SqlPro()
  public RationHourUnit: string = '1'
  public ConnectString: string = ''
  public ServerUri: string = 'ws://192.168.1.15:9000'
  //public client: Socket | null = null;
  public iProgress: number = 0
  public iBatchRowCount: number = 50000
  public name: string = 'GetSchOperationProgress'
  public schPrecent: number
  public message: string
  public User: string = 'admin'
  public Company: string = '001'
  public socketId: string = '123'
  datasource: DataSource
  //public dlShowProcess: DLShowProcess;

  constructor(config: { datasourcce: DataSource }) {
    super()
    this.datasource = config.datasourcce //
    //this.dlShowProcess = this.ShowProcess.bind(this);
  }

  public async SetInit(input: any): Promise<Object> {
    let ConnectString: string = input.ConnectString
  ...
  public async SetInitB(
    ConnectString: string,
    ServerUri: string = 'ws://192.168.1.15:9000',
    Company: string = '',
    cUser: string = 'admin',
  ): Promise<string> {
    //APSRun.SqlPro.ConnectionStrings = ConnectString;
...
//   }
  //后端Sch调用此函数
  public async Start(input: any): Promise<any> {
    let SqlPro = this //
    let cVersionNo: string = input.cVersionNo
    let adte_Start: DateTime = new DateTime(input.adte_Start)
    let adte_End: DateTime = new DateTime(input.adte_End)
    let bShowTips: boolean = true
    let cSchType: string = input.cSchType
    let cTechSchType: string = input.cTechSchType
    let iCurSchRunTimes: string = input.iCurSchRunTimes
    debugger
    ...
    this.schData.dtToday = new DateTime() // Assuming this is the current date
    this.schData.cCalculateNo = `${this.User};${this.schData.dtToday}`
    console.log(this.schData.dtToday)
    let SchParam = new _SchParam() //系统参数，实例化，后续都用这个类
    ...
    // SchParam.dtParamValue = await GetDataTable('select * from t_ParamValue where 1 = 1 ', '');
    // SchParam.GetSchParams();
    //待编写
    await SchParam.GetSchParams()
    ...
    if (SchParam.bReSchedule !== '1') {
      ...
      this.dlShowProcess(this.schPrecent.toString(), this.message)
      try {
        let lsSql2 = `EXECUTE P_SchedulePrePre '${
          this.schData.cVersionNo
        }','${this.schData.dtStart.toString()}','${this.schData.dtEnd.toString()}','${
          SchParam.cSchType
        }','${SchParam.cTechSchType}','${this.schData.cCalculateNo}','${
          SchParam.iCurSchRunTimes
        }'`
        await SqlPro.ExecuteNonQuery(lsSql2, null)
        let lsSql = `EXECUTE P_SchedulePre '${
          this.schData.cVersionNo
        }','${this.schData.dtStart.toString()}','${this.schData.dtEnd.toString()}','${
          SchParam.cSchType
        }','${SchParam.cTechSchType}','${this.schData.cCalculateNo}','${
    ...
    try {
      ...
      this.dlShowProcess(this.schPrecent.toString(), this.message)
      console.log(
        '排产计算出错！位置2、生成资源工作列表及资源工作日历,出错内容：' +
          excp.toString(),
      )
      throw excp
    }
    ...
    this.schPrecent = this.schData.iProgress
    this.dlShowProcess(this.schPrecent.toString(), this.message)
    if ((await this.GetSchData()) < 0) return -1
    ...
    if ((await this.ResSchTaskInit()) < 0) return -1
    ...
    try {
      ...
      // let threadRows = new Promise<void>((resolve) => {
      // 	this.ShowSchProgress();
      // 	resolve();
      // });
      // await threadRows;
      if ((await scheduling.SchMainRun(cSchType)) < 0) return -1
    } catch (excp) {
      this.message =
        '排产计算出错！位置1、排产前处理出错,出错内容：' + excp.toString()
      this.schPrecent = 100
    ...
    this.dlShowProcess(this.schPrecent.toString(), this.message)
    try {
      let lsSql = `EXECUTE P_SchedulePost '${
        this.schData.cVersionNo
      }','${this.schData.dtStart.toString()}','${this.schData.dtEnd.toString()}','${this.schData.dtToday.toString()}','${
        SchParam.cSchType
      }','${SchParam.cTechSchType}','${this.schData.cCalculateNo}','${
        SchParam.iCurSchRunTimes
      }'`
      ...
      console.log(
        '排产计算出错！位置GetResourceList(),出错内容：' + excp.toString(),
      )
      throw excp
    }
    ...
    try {
      let lsSql = `EXECUTE P_SchKPI '${this.schData.cVersionNo}','${
        this.ldtBegDate
      }','${new DateTime()}','${this.schData.cCalculateNo}','','${
        SchParam.cSchType
      }','${SchParam.cTechSchType}'`
      await SqlPro.ExecuteNonQuery(lsSql, null)
    } catch (excp) {
  ...
  public static IsNullable(t: any): boolean {
    return t == null || t == undefined
    // return !t.isValueType || (t.isGenericType && t.getGenericTypeDefinition() === Nullable);
  }

  public static GetCoreType(t: any): any {
  ...
  private async GetSchData(): Promise<number> {
    // Implementation here
    return 0
  }

  private async ResSchTaskInit(): Promise<number> {
    // Implementation here
    return 0
  }

  private async SaveSchData(): Promise<number> {
    // Implementation here
    return 0
  }

  private ShowProcess(schPrecent: string, message: string): void {
    try {
  ...
  private SendAsync(message: string): void {
    // Implementation for sending messages
  }

  //
  private ShowSchProgress(): void {
    // Implementation for showing schedule progress
  }
  //返回dataTable对象
  // 	async GetDataTable(str: string, param: any) {

  //   }
  // 	//
  public GetResourceList(): number {
    let SqlPro = this
    let SchParam: any = console.log('取生成了资源工作日历的资源列表') //SchParam 系统参数类
    let lsResAddWhere: string =
      " and cStatus <> '3' and (cResourceNo in (select distinct cResourceNo "
    let lstg_Sql: string = `EXECUTE P_GetSchDataResource '${
      this.schData.cVersionNo
    }', '${this.schData.dtStart.addDays(-20)}', '${this.schData.dtEnd}', '${
      SchParam.cSchType
    }', '${SchParam.cTechSchType}', '${this.schData.cCalculateNo}'`
    this.schData.dtResource = SqlPro.GetDataTable(lstg_Sql, null)
    let lsSql: string =
      "SELECT FProChaTypeID,FResChaTypeName,cParentClsNo,bEnd,cBarCode,cNote,cMacNo FROM t_ProChaType where isnull(bEnd,1) = '1'"
    ...
    try {
      console.log(
        'schData.dtStart - schData.dtEnd' +
          this.schData.dtStart.addDays(-20) +
          this.schData.dtEnd,
      )
      lstg_Sql = `EXECUTE P_GetResWorkTime '${
        this.schData.cVersionNo
      }','${this.schData.dtStart.addDays(-20)}','${this.schData.dtEnd}','${
        SchParam.cSchType
      }','${SchParam.cTechSchType}','${this.schData.cCalculateNo}'`
      //@ts-ignore
      let dt_ResourceTime: DataTable = SqlPro.GetDataTable(lstg_Sql, null)
      ...
      let message: string = '2.1、资源工作日历生成完成'
      let schPrecent: number = this.schData.iProgress
      ...
      let dt_ResourceSpecTime: DataTable = SqlPro.GetDataTable(lstg_Sql, null)
      this.schData.dt_ResourceSpecTime = dt_ResourceSpecTime
      this.schData.dt_ResourceSpecTime.DefaultView.Sort = 'dPeriodDay asc'
      let cWorkCenter: string
      let lobj_WorkCenter: WorkCenter
      console.log('3、生成工作中心')
      for (let drWorkCenter of this.schData.dtWorkCenter.rows) {
        cWorkCenter = drWorkCenter['cWcNo'].toString()
        lobj_WorkCenter = new WorkCenter(cWorkCenter, this.schData)
        this.schData.WorkCenterList.push(lobj_WorkCenter)
      }
      let cResourceNoOld: string = ''
      let cResourceNo: string
      let lobj_Resource: Resource
      let lResTimeRange: ResTimeRange
      let ResTimeRangeType: number
      ...
      for (let drResource of this.schData.dtResource.rows) {
        ...
        lobj_Resource = new Resource(cResourceNo)
        this.schData.ResourceList.push(lobj_Resource)
        if (lobj_Resource.bTeamResource !== '1') {
          let drResTime: DataRow[] = dt_ResourceTime.Select(
          ...
          for (let drResTimeRange of drResTime) {
            ...
            lResTimeRange.CIsInfinityAbility = drResource[
              'cIsInfinityAbility'
            ].toString()
            lResTimeRange.iPeriodID = <number>drResTimeRange['iPeriodTimeID']
            ...
            lResTimeRange.Attribute = parseInt(
              drResTimeRange['cTimeRangeType'].toString(),
            )
            lResTimeRange.GetNoWorkTaskTimeRange(
              lResTimeRange.DBegTime,
              lResTimeRange.DEndTime,
              true,
            )
            lobj_Resource.ResTimeRangeList.push(lResTimeRange)
          }
          let drResSpecTime: DataRow[] = dt_ResourceSpecTime.Select(
            `cResourceNo = '${cResourceNo}'`,
          )
          for (let drResTimeRange of drResSpecTime) {
            ...
            lResTimeRange.CResourceNo = cResourceNo
            lResTimeRange.resource = lobj_Resource
            lResTimeRange.iPeriodID = parseInt(
              drResTimeRange['iPeriodTimeID'].toString(),
            )
            lResTimeRange.CIsInfinityAbility = drResource[
              'cIsInfinityAbility'
            ].toString()
            ...
            lResTimeRange.HoldingTime =
              parseInt(
                parseFloat(
                  drResTimeRange['iHoldingTime'].toString(),
                ).toString(),
              ) * 60
      ...
      this.schData.KeyResourceList = this.schData.ResourceList.filter(
        (p1) => p1.cIsKey === '1' && p1.iKeySchSN > 0,
      )
      this.schData.KeyResourceList.sort((p1, p2) => p1.iKeySchSN - p2.iKeySchSN)
      this.schData.TeamResourceList = this.schData.ResourceList.filter(
        (p1) => p1.bTeamResource === '1',
      )
      if (this.schData.TeamResourceList.length > 0) {
        for (let TeamResource of this.schData.TeamResourceList) {
          let ResourceSubList: Resource[] = this.schData.ResourceList.filter(
            (p1) => p1.cTeamResourceNo === TeamResource.cResourceNo,
          )
      ...
      console.log(
        '排产计算出错！位置GetResourceList(),出错内容：' + ex1.toString(),
      )
      throw ex1
    }
    return 1
  }
  public async getSchData(): Promise<number> {
    console.log('开始第二步骤')
    ...
    try {
      ...
      lstg_Sql = `EXECUTE P_GetSchProductWorkItem '${
        schData.cVersionNo
      }', '${schData.dtStart.addDays(-20)}', '${schData.dtEnd}', '${
        SchParam.cSchType
      }', '${SchParam.cTechSchType}', '${this.schData.cCalculateNo}'`
      ...
      schData.dtSchProductWorkItem.forEach((dr) => {
        ...
        lSchProductWorkItem.dCanBegDate = dr.dCanBegDate ?? new DateTime()
      ...
      lstg_Sql = `EXECUTE P_GetSchDataProduct '${
        schData.cVersionNo
      }', '${schData.dtStart.addDays(-20)}', '${schData.dtEnd}', '${
        SchParam.cSchType
      }', '${SchParam.cTechSchType}', '${this.schData.cCalculateNo}'`
      ...
      schData.dtSchProduct.forEach((dr) => {
        ...
        lSchProduct.dEarliestSchDate = dr.dEarliestSchDate ?? new DateTime()
      ...
      lstg_Sql = `EXECUTE P_GetSchDataProductRoute '${
        schData.cVersionNo
      }', '${schData.dtStart.addDays(-20)}', '${schData.dtEnd}', '${
        SchParam.cSchType
      }', '${SchParam.cTechSchType}', '${this.schData.cCalculateNo}'`
      ...
      schData.dtSchProductRoute.forEach((dr) => {
        ...
        lSchProductRoute.item = schData.ItemList.find(
          (p) => p.cInvCode == lSchProductRoute.cWorkItemNo,
        )
        lSchProductRoute.iProcessID = dr.iProcessID
        lSchProductRoute.iWoSeqID = dr.iWoSeqID
        lSchProductRoute.cTechNo = dr.cTechNo
        lSchProductRoute.techInfo = schData.TechInfoList.find(
          (p) => p.cTechNo == lSchProductRoute.cTechNo,
        )
      ...
      lstg_Sql = `EXECUTE P_GetSchDataProductRouteRes '${
        schData.cVersionNo
      }', '${schData.dtStart.addDays(-20)}', '${schData.dtEnd}', '${
        SchParam.cSchType
      }', '${SchParam.cTechSchType}', '${this.schData.cCalculateNo}'`
      ...
      schData.dtSchProductRouteRes.forEach((dr) => {
        ...
        lSchProductRouteRes.resource = schData.ResourceList.find(
          (p) => p.cResourceNo == lSchProductRouteRes.cResourceNo,
        )
        if (lSchProductRouteRes.resource == null) {
          throw new Error(
            `排程ID[${lSchProductRouteRes.iSchSdID}, ${lSchProductRouteRes.iProcessProductID}] 资源编号[${lSchProductRouteRes.cResourceNo}]不存在,注意区分大小写和空格,或者资源没有定义工作日历!`,
          )
        }
        if (
          lSchProductRouteRes.resource.FProChaType1ID != '-1' &&
          lSchProductRouteRes.resource.FProChaType1ID != ''
        )
          lSchProductRouteRes.resChaValue1 = new ResChaValue(
            lSchProductRouteRes.FResChaValue1ID,
            lSchProductRouteRes,
            1,
          )
        if (
          lSchProductRouteRes.resource.FProChaType2ID != '-1' &&
          lSchProductRouteRes.resource.FProChaType2ID != ''
        )
          lSchProductRouteRes.resChaValue2 = new ResChaValue(
            lSchProductRouteRes.FResChaValue2ID,
            lSchProductRouteRes,
            2,
          )
        if (
          lSchProductRouteRes.resource.FProChaType3ID != '-1' &&
          lSchProductRouteRes.resource.FProChaType3ID != ''
        )
          lSchProductRouteRes.resChaValue3 = new ResChaValue(
            lSchProductRouteRes.FResChaValue3ID,
            lSchProductRouteRes,
            3,
          )
        if (
          lSchProductRouteRes.resource.FProChaType4ID != '-1' &&
          lSchProductRouteRes.resource.FProChaType4ID != ''
        )
          lSchProductRouteRes.resChaValue4 = new ResChaValue(
            lSchProductRouteRes.FResChaValue4ID,
            lSchProductRouteRes,
            4,
          )
        if (
          lSchProductRouteRes.resource.FProChaType5ID != '-1' &&
          lSchProductRouteRes.resource.FProChaType5ID != ''
        )
          lSchProductRouteRes.resChaValue5 = new ResChaValue(
            lSchProductRouteRes.FResChaValue5ID,
            lSchProductRouteRes,
            5,
          )
        if (
          lSchProductRouteRes.resource.FProChaType6ID != '-1' &&
          lSchProductRouteRes.resource.FProChaType6ID != ''
        )
          lSchProductRouteRes.resChaValue6 = new ResChaValue(
            lSchProductRouteRes.FResChaValue6ID,
            lSchProductRouteRes,
            6,
          )
      ...
      if (SchParam.cSelfEndDate == '1') {
        schData.dtSchProductRouteItem = await this.getSqlDataTable(
          lsSchProductRouteItem,
        )
        ...
        schData.dtSchProductRouteItem.forEach((dr) => {
          ...
          if (lSchProductRouteItem.iProcessProductID < 0) return
          ...
          lSchProductRouteItem.schProductRoute = schData.SchProductRouteList.find(
            (p) =>
              p.iProcessProductID == lSchProductRouteItem.iProcessProductID,
          )
      ...
      schData.SchProductRouteList.forEach((lSchProductRoute) => {
        lSchProductRoute.SchProductRouteResList = schData.SchProductRouteResList.filter(
          (p) =>
            p.iSchSdID == lSchProductRoute.iSchSdID &&
            p.iProcessProductID == lSchProductRoute.iProcessProductID,
        )
        if (lSchProductRoute.SchProductRouteResList.length > 0) {
          lSchProductRoute.SchProductRouteResList.forEach(
            (lSchProductRouteRes) => {
        ...
        if (SchParam.cSelfEndDate == '1') {
          lSchProductRoute.SchProductRouteItemList = schData.SchProductRouteItemList.filter(
            (p) =>
              p.iSchSdID == lSchProductRoute.iSchSdID &&
              p.iProcessProductID == lSchProductRoute.iProcessProductID,
          )
        ...
        if (lSchProductRoute.cVersionNo != 'SureVersion') {
          let ListRouteRes = lSchProductRoute.SchProductRouteResList.filter(
            (p) => p.cSelected == '1',
          )
          let iRandom: number
          let iCount = ListRouteRes.length,
            iRowCount = ListRouteRes.length
      ...
      schData.SchProductList.forEach((lSchProduct) => {
        lSchProduct.SchProductRouteList = schData.SchProductRouteList.filter(
          (p) => p.iSchSdID == lSchProduct.iSchSdID,
        )
      ...
      schData.SchProductWorkItemList.forEach((lSchProductWorkItem) => {
        lSchProductWorkItem.SchProductRouteList = schData.SchProductRouteList.filter(
          (p) =>
            p.iSchSdID == lSchProductWorkItem.iSchSdID &&
      ...
      let SchProductRouteList = schData.SchProductRouteList.filter(
        (p1) => p1.cDevCountPdExp != '' && p1.cWoNo == '',
      )
  ...
  getSchProductRouteList(lSchProductRoute: SchProductRoute, arg1: boolean) {
    throw new Error('Method not implemented.')
  }

  async getSqlDataTable(query: string): Promise<any> {
    let table = new DataTable()
    await table.executeQuery(query)
    return table
    //   let datasource=this.datasource
    //   let rows=datasource.query(query)
  ...
  // 	let lstg_Sql = `EXECUTE P_GetSchDataResource '${this.schData.cVersionNo}', '${this.schData.dtStart.addDays(
  // 		-20,
  // 	)}', '${this.schData.dtEnd}', '${SchParam.cSchType}', '${SchParam.cTechSchType}', '${this.schData.cCalculateNo}'`;
  // 	this.schData.dtResource = SqlPro.getDataTable(lstg_Sql, null);
  // 	let lsSql =
  // 		"SELECT FProChaTypeID,FResChaTypeName,cParentClsNo,bEnd,cBarCode,cNote,cMacNo FROM t_ProChaType where isnull(bEnd,1) = '1'";
  // 	this.schData.dtProChaType = SqlPro.getDataTable(lsSql, null);
  ...
  // 	console.log('更新未排订单排程可开始时间');
  // 	try {
  // 		console.log('schData.dtStart - schData.dtEnd' + this.schData.dtStart.addDays(-20) + this.schData.dtEnd);
  // 		lstg_Sql = `EXECUTE P_GetResWorkTime '${this.schData.cVersionNo}','${this.schData.dtStart.addDays(-20)}','${
  // 			this.schData.dtEnd
  // 		}','${SchParam.cSchType}','${SchParam.cTechSchType}','${this.schData.cCalculateNo}'`;
  // 		let dt_ResourceTime: DataTable = SqlPro.getDataTable(lstg_Sql, null);
  // 		this.schData.dt_ResourceTime = dt_ResourceTime;
  ...
  // 		this.schData.dt_ResourceSpecTime.defaultView.sort = 'dPeriodDay asc';
  // 		let cWorkCenter: string;
  // 		let lobj_WorkCenter: Algorithm.WorkCenter;
  // 		console.log('3、生成工作中心');
  // 		for (let drWorkCenter of this.schData.dtWorkCenter.rows) {
  // 			cWorkCenter = drWorkCenter['cWcNo'].toString();
  ...
  // 					lResTimeRange.DBegTime = new DateTime(drResTimeRange['dPeriodBegTime']);
  // 					lResTimeRange.DEndTime = new DateTime(drResTimeRange['dPeriodEndTime']);
  // 					lResTimeRange.dPeriodDay = new DateTime(drResTimeRange['dPeriodDay']);
  // 					lResTimeRange.FShiftType = drResTimeRange['FShiftType'].toString();
  // 					if (lResTimeRange.FShiftType === '') {
  // 						lResTimeRange.FShiftType = 'A班'; //班次 A班 夜班 中班等
  // 					}
  // 					lResTimeRange.Attribute = <Algorithm.TimeRangeAttribute>(
  // 						parseInt(drResTimeRange['cTimeRangeType'].toString())
  // 					);
  ...
  // 						parseInt(drResTimeRange['cTimeRangeType'].toString())
  // 					);
  // 					lResTimeRange.HoldingTime = parseInt(parseFloat(drResTimeRange['iHoldingTime'].toString())) * 60;
  // 					lobj_Resource.ResSpecTimeRangeList.push(lResTimeRange);
  // 				}
  // 				if (drResSpecTime.length > 0) {
  // 					lobj_Resource.mergeTimeRange();
  ...
  // 		if (this.schData.TeamResourceList.length > 0) {
  // 			for (let TeamResource of this.schData.TeamResourceList) {
  // 				let ResourceSubList = this.schData.ResourceList.filter(
  // 					(p1) => p1.cTeamResourceNo === TeamResource.cResourceNo,
  // 				);
  // 				TeamResource.TeamResourceList = ResourceSubList;
  // 				for (let TeamResourceSub of ResourceSubList) {
  ...
  // }
  //待实现
  dlShowProcess(schPrecent: string, message: string) {
    console.log(schPrecent + ' ' + message) //显示进度
  }
}

export const createInteface=(input:any,datasource:any)=>{
