...
export class SchProductRoute extends Base {
  ...
  cPreProcessItem: string
  cPostProcessItem: string
  iAutoID: number
  cLevelInfo: string
  iLevel: number
  iParentItemID: number
  cParentItemNo: string
  cParentItemNoFull: string
  cParellelType: string
  cParallelNo: string
  cKeyBrantch: string
  cCompSeq: string
  cMoveType: string
  iMoveInterTime: number
  iMoveInterQty: number
  iMoveTime: number
  iDevCountPd: number = 1
  cDevCountPdExp: string
  private ISeqPretime: number
  private ISeqPostTime: number
  iCapacity: number
  cCapacityExp: string
  ...
  dBegDate: any
  dEndDate: any
  dActBegDate: DateTime
  dActEndDate: DateTime
  dEarlyBegDate: DateTime
  ...
  cDefine37: DateTime
  iSchBatch: number = 6
  cBatchNoFlag: number = 0
  dCanBegDate: DateTime
  dFirstBegDate: DateTime
  dFirstEndDate: DateTime
  schProduct: SchProduct
  schProductWorkItem: SchProductWorkItem
  item: Item
  techInfo: TechInfo
  SchProductRouteItemList: SchProductRouteItem[] = new Array(10)
  SchProductRouteResList: SchProductRouteRes[] = new Array(10)
  SchProductRoutePreList: SchProductRoute[] = new Array(10)
  SchProductRouteNextList: SchProductRoute[] = new Array(10)
  ...
  ProcessSchTask(bFreeze: boolean = false) {
    ...
    try {
      ...
      if (
        ...
        //@ts-ignore
        dCanBegDate = this.dFirstBegDate.setDate(
          this.dFirstBegDate.getDate() - SchParam.dLastBegDateBeforeDays,
        )
      }
      this.GetRouteEarlyBegDate()
      if (dCanBegDate < this.dEarlyBegDate) dCanBegDate = this.dEarlyBegDate
    } catch (error) {}
    if (this.cVersionNo.toLowerCase() == 'sureversion') {
      ...
      const ListRouteRes = this.SchProductRouteResList.filter(
        (p) => p.cSelected == '1' && p.iResReqQtyOld > 0,
      )
      if (
        this.item != null &&
        ...
        for (let i = 0; i < ListRouteRes2.length; i++) {
          ...
          ListRouteRes2[i].dResEndDate = new DateTime(dCanBegDate.getTime() + 60000)
      ...
      for (let i = 0; i < ListRouteRes.length; i++) {
        if (ListRouteRes[i].iResReqQty > 0)
          ...
          ListRouteRes[i].dResEndDate = new DateTime(dCanBegDate.getTime() + 60000)
      ...
      try {
        if (this.iReqQty > 0) {
          ...
          if (
            ...
            for (let i = 0; i < ListRouteRes2.length; i++) {
              ListRouteRes2[i].dResBegDate = dCanBegDate
              ListRouteRes2[i].dResEndDate = new DateTime(
                dCanBegDate.getTime() + 60000,
              )
              ListRouteRes2[i].TaskTimeRangeList.length = 0
              ListRouteRes2[i].BScheduled = 1
            }
          ...
          for (let i = 0; i < ListRouteRes2.length; i++) {
            if (ListRouteRes2[i].iResReqQty > 0)
              ListRouteRes2[i].TaskSchTask(
                ListRouteRes2[i].iResReqQty,
                dCanBegDate,
              )
            else {
              ListRouteRes2[i].dResBegDate = dCanBegDate
      ...
      if (iResCount < 1) {
        const ListRouteResCan = this.SchProductRouteResList.filter(
          (p) => p.cSelected == '1' && p.cCanScheduled == '0',
        )
      ...
      for (let i = 0; i < ListRouteRes.length; i++) {
        ...
        try {
          const ldtBeginDate = new DateTime()
          ListRouteRes[i].TaskSchTask(iResReqQty, dCanBegDate)
          const ldtEndedDate = new DateTime()
          const iWaitTime2 = new DateTime().getTime() - ldtBeginDate.getTime()
          ...
          this.SchProductRoutePreList.forEach((schProductRoutePre) => {
            ...
            if (
              ...
              const iDiffMinites =
                SchData.GetDateDiff(
                  'm',
                  schProductRoutePre.dEndDate,
                  ListRouteRes[i].dResEndDate,
                ) -
    ...
    const list1 = this.SchProductRouteResList.filter(
      (p1) =>
    ...
    if (list1.length > 0) {
      ...
      const list2 = this.SchProductRouteResList.filter(
        (p1) =>
          p1.iSchSdID == this.iSchSdID &&
          p1.iProcessProductID == this.iProcessProductID &&
          p1.iResReqQty > 0,
  ...
  ResourceSelect(dCanBegDate: DateTime, bFreeze: boolean = false): number {
    let Comparer=this.Comparer
    let SchParam = this.SchParam
    let ListRouteRes = this.SchProductRouteResList.filter(
      (p) => p.cSelected == '1',
    )
    if (
      this.iSchSdID == SchParam.iSchSdID &&
    ...
    if (iRouteResCountFirst < 1) {
      throw new Error(
        `多资源选择正排出错,订单行号：${this.iSchSdID}，没有找到已选择的可排产资源，资源产能明细总资源数量为${this.SchProductRouteResList.length}，位置SchProductRoute.ProcessSchTask！工序ID号：${this.iProcessProductID}，物料编号${this.cInvCode}`,
      )
      return -1
    }
    ...
    ListRouteRes.sort((p1, p2) =>
      Comparer.Default.Compare(
        p1.iCapacity / p1.iBatchQty,
        p2.iCapacity / p1.iBatchQty,
      ),
    )
    ...
    if (ListRouteRes.length > 1) {
      if (
        ...
        else iMinResCount = iResCount
      }
    }
    if (iResCount > iMinResCount && iMinResCount <= ListRouteRes.length)
      iResCount = iMinResCount
    if (iResCount < 1 && ListRouteRes.length > 0) iResCount = 1
    if (
      this.item != null &&
    ...
    if (iSelectReturn <= 0) {
      if (SchParam.cMutResourceType == '4') {
        ListRouteRes.sort((p1, p2) =>
          Comparer.Default.Compare(
            p1.resource.iPlanDays,
            p2.resource.iPlanDays,
          ),
        )
        ...
        if (ListRouteRes.length > 1) {
          for (let i = 0; i < ListRouteRes.length; i++) {
            if (ListRouteRes[i].cCanScheduled != '1') continue
            ListRouteRes[i].cDefine38 = new DateTime(
              ListRouteRes[i].dResEndDate.getTime() +
          ...
          const ListRouteResSort = ListRouteRes.sort(
            (c1, c2) =>
              Comparer.Default.Compare(c1.cDefine38, c2.cDefine38) ||
              Comparer.Default.Compare(
                c1.iResGroupPriority,
                c2.iResGroupPriority,
              ),
          )
        ...
        ListRouteRes.sort((p1, p2) =>
          Comparer.Default.Compare(p1.dResEndDate, p2.dResEndDate),
        )
      }
      const ResSelectList: string[] = []
      const MoldSelectList: string[] = []
      if (ListRouteRes.length > 1) {
        let j = 0
        for (let i = 0; i < ListRouteRes.length; i++) {
          if (ListRouteRes[i].cCanScheduled != '1') continue
          if (j <= iResCount) {
            if (ResSelectList.length > 0) {
              const ResSelect = ResSelectList.find((s: any) =>
                s.equals(ListRouteRes[i].cResourceNo),
              )
              if (
                ResSelectList.length > 0 &&
                ResSelect != null &&
                ResSelect != ''
              ) {
                ListRouteRes[i].cCanScheduled = '0'
    ...
    ListRouteRes = this.SchProductRouteResList.filter(
      (p) => p.cSelected == '1' && p.cCanScheduled == '1',
    )
    iResCount = ListRouteRes.length
    if (iResCount < 1) {
      throw new Error(
        `多资源选择正排出错,订单行号：${this.iSchSdID}，没有找到已选择的可排产资源，资源产能明细总资源数量为${this.SchProductRouteResList.length}，初始选择可排资源数量为${iRouteResCountFirst}，位置SchProductRoute.ProcessSchTask！工序ID号：${this.iProcessProductID}，物料编号${this.cInvCode}`,
      )
      return -1
    }
  ...
  ResReqQtyDispatch(): number {
    ...
    for (let i = 0; i < ListRouteRes.length; i++) {
      ...
      if (i == ListRouteRes.length) iResReqQty = iLeftReqQty
      else {
  ...
  ResourceSelectRev(dCanBegDate: DateTime, bFreeze: boolean = false): number {
    let Comparer=this.Comparer
    let ListRouteRes = this.SchProductRouteResList.filter(
      (p) => p.cSelected == '1',
    )
    ...
    if (iResCount > this.iDevCountPd && this.iDevCountPd >= 1)
      iResCount = this.iDevCountPd
    ListRouteRes.sort((p1, p2) =>
      Comparer.Default.Compare(p1.iCapacity, p2.iCapacity),
    )
    let iCapacity = ListRouteRes[0].iCapacity
    let iMinResCount = 1
    if (ListRouteRes.length > 1 && this.cDevCountPdExp.length < 1) {
      if (
        ...
        if (SchParam.iTaskMinWorkTime > 0)
          iMinResCount = Math.floor(
            (this.iReqQty * iCapacity) / 3600 / SchParam.iTaskMinWorkTime,
          )
        else iMinResCount = iResCount
      }
    }
  ...
  cBatchResourceSelect(): number {
    this.cBatchNoFlag = 1
    const ListBatchRoute = this.schData!.SchProductRouteList.filter(
      (p) =>
    ...
    ListBatchRoute.forEach((batchRoute) => {
      for (let i = 0; i < batchRoute.SchProductRouteResList.length; i++) {
        if (
          batchRoute.SchProductRouteResList[i].resource.cIsInfinityAbility ==
          '0'
        ) {
  ...
  ProcessSchTaskPre(
    ...
    try {
      ...
      if (SchParam.iSchSdID < 1) {
        throw new Error(
          `订单行号：${this.iSchSdID}资源倒排计算时出错,位置SchProductRoute.ProcessSchTaskNext！工序ID号：${this.iProcessProductID}\n\r ${error.message}`,
        )
      } else {
        throw new Error(
  ...
  ProcessSchTaskNext(cTag: string = '1'): number {
    try {
      ...
      if (!schProductRouteNext) {
        throw new Error(
          `订单行号：${this.iSchSdID}请检查产品[${this.cInvCode}]加工物料[${
            this.cWorkItemNo
          }]工序号[${this.iWoSeqID.toString()}]工艺路线是否完整!`,
        )
        return -1
      }
      if (cTag === '1') {
  ...
  ProcessSchTaskRevPre(cTag: string = '1', bSet: string = '0'): number {
    if (cTag === '2') {
  ...
  ProcessSchTaskRev(bSet: string = '0'): number {
    const SchParam = this.SchParam
    let dDateTemp = this.schData.dtStart
    ...
    try {
      if (this.SchProductRouteNextList.length > 0) {
        ...
        if (dCanEndDate < this.schData.dtStart) {
          throw new Error(
            `订单行号：${this.iSchSdID}资源倒排计算时出错,位置SchProductRoute.ProcessSchTaskRev！订单行号：${this.iSchSdID}工序ID号：${this.iProcessProductID}\n\r 倒排开始时间${dCanEndDate}小于排产开始日期${this.schData.dtStart},无法继续倒排!`,
          )
          return -1
        }
      } else {
        ...
        if (ListRouteResCan.length < 1) {
          throw new Error(
            `订单行号：${
              ...
              this.iProcessProductID
            }\n\r 没有可排产资源编号.或排产范围太小,最早可排开始日期：'${dCanEndDate.toLocaleString(
              this.DateTime.DATE_FULL,
            )}'`,
          )
          return -1
        } else {
      ...
      for (const LSchProductRouteRes of ListRouteRes) {
        LSchProductRouteRes.TaskSchTaskRev(
          LSchProductRouteRes.iResReqQty,
          dCanEndDate,
        )
        if (
          this.SchProductRouteNextList &&
          this.SchProductRouteNextList.length > 0
        ) {
          if (
            LSchProductRouteRes.dResBegDate
              //@ts-ignore
              .diff(this.SchProductRouteNextList[0].dBegDate)
              .as('hours') > 0
          ) {
            const iDiffMinites = SchData.GetDateDiff(
              'm',
              this.SchProductRouteNextList[0].dBegDate,
              LSchProductRouteRes.dResBegDate,
            )
            if (iDiffMinites > 0) {
              //@ts-ignore
              ...
              try {
                ...
                if (SchParam.iSchSdID < 1) {
                  throw new Error(
                    `订单行号：${this.iSchSdID}倒排时，资源正排工序完工时间比前工序早,需倒排计算时出错,位置SchProductRoute.ProcessSchTask！工序ID号：${this.iProcessProductID}\n\r ${error.message}`,
                  )
                } else {
                  throw new Error(
                    `订单行号：${this.iSchSdID}倒排时，资源正排工序完工时间比前工序早,需倒排计算时出错,位置SchProductRoute.ProcessSchTask！工序ID号：${this.iProcessProductID}\n\r ${error.message} 明细信息:${error.stack}`,
  ...
  GetNextProcessCanBegDate(schProductRouteNext: SchProductRoute): DateTime {
    ...
    try {
      ...
      if (this.cParellelType === 'ES') {
        if (SchParam.NextSeqBegTime > 0) {
          ldtFirstEndDate = this.dBegDate.plus({
            minutes:
              this.iMoveTime + this.iSeqPostTime + SchParam.NextSeqBegTime,
          })
          if (
            ldtFirstEndDate >
        ...
        if (schProductRouteNext.BScheduled === 1) {
          if (this.cMoveType === '1') {
            ...
            if (cWorkType === '1') {
              ldtFirstEndDate = schProductRouteNext.dEndDate.minus({
                minutes:
                  (this.iMoveInterQty * liCapacity) / 60 / iBatchQty +
                  this.iMoveTime +
                  this.iSeqPostTime,
              })
            } else {
              ldtFirstEndDate = schProductRouteNext.dEndDate.minus({
                minutes:
                  (this.iMoveInterQty * liCapacity) / 60 +
                  this.iMoveTime +
                  this.iSeqPostTime,
              })
            }
      ...
      throw new Error(
        `订单行号：${this.iSchSdID}资源计算时出错,位置SchProductRoute.GetNextProcessCanBegDate！订单行号：${this.iSchSdID}工序ID号：${this.iProcessProductID}\n\r ${error.message} ${error.stack}`,
      )
    }
    return ldtFirstEndDate
  }

  GetProcessMoveTime(schProductRoutePre: SchProductRoute): number {
    let iMoveTime = 0
    let liCapacity = 1
  ...
  GetProcessMoveQty(schProductRoutePre: SchProductRoute): number {
    let iMoveTime = 0
    let liCapacity = 1
    let ibatchqty = 1
    if (schProductRoutePre.SchProductRouteResList.length > 0) {
  ...
  GetPreProcessCanEndDate(
    schProductRoutePre: SchProductRoute,
    cType: string = '1',
  ): DateTime {
    let ldtFirstEndDate: DateTime
    const iMoveTime = this.GetProcessMoveQty(schProductRoutePre)
    ...
    try {
      ...
      if (SchParam.cPurEndDate === '1' && this.cVersionNo !== 'SureVersion') {
        if (
          ...
          const SchProductRouteItemListPur = this.SchProductRouteItemList.filter(
            (p) =>
              p.bSelf !== '1' &&
