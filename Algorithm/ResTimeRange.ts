...
export class ResTimeRange extends Base {
  ...
  dBegTime: DateTime = new DateTime() // 时间段开始时间
  dEndTime: DateTime = new DateTime() // 时间段结束时间
  ...
  dPeriodDay: DateTime = new DateTime() // 时段所属日期
  ...
  TimeSchTaskFreezeInit(
    ...
    try {
      for (let i = 0; i < NoTimeRangeList.length; i++) {
        ...
        if (taskTimeRange1.DBegTime < new DateTime('2011-01-01')) {
      ...
      for (let i = 0; i < NoTimeRangeList.length; i++) {
        ...
        if (NoTimeRange.dBegTime >= adCanBegDate) {
          if (ai_workTime > NoTimeRange.AvailableTime) {
            ...
            // 部分占用
            taskTimeRange1.DBegTime = NoTimeRange.DBegTime
            taskTimeRange1.DEndTime = new DateTime(
  ...
  TimeSchTask(
    ...
    let dtiDayDis = new DateTime(adCanBegDate.getTime() - 86400000) // 学习曲线日期
      ...
      for (let i = 0; i < NoTimeRangeList.length; i++) {
        ...
        if (NoTimeRange.DEndTime <= adCanEndDate) {
          if (ai_workTime > NoTimeRange.AvailableTime) {
            ...
            taskTimeRange1.DBegTime = new DateTime(
  ...
  GetAvailableTime(adCanBegDate: DateTime, bSchRev: boolean = false): DateTime {
  ...
  get DBegTime(): DateTime {
    return this.dBegTime
  }

  set DBegTime(value: DateTime) {
